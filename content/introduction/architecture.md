...
CadenzaFlow is a Java-based framework. The main components are written in Java and we have a general focus on providing Java developers with the tools they need for designing, implementing and running business processes and workflows on the JVM. Nevertheless, we also want to make the process engine technology available to non-Java developers. This is why CadenzaFlow also provides a REST API which allows you to build applications connecting to a remote process engine.

CadenzaFlow can be used both as a standalone process engine server or embedded inside custom Java applications. The embeddability requirement is at the heart of many architectural decisions within CadenzaFlow. For instance, we work hard to make the process engine component a lightweight component with as little dependencies on [third-party libraries]({{< ref "/introduction/third-party-libraries/_index.md" >}}) as possible. Furthermore, the embeddability motivates programming model choices such as the capabilities of the process engine to participate in Spring Managed or JTA [transactions and the threading model]({{< ref "/user-guide/process-engine/transactions-in-processes.md" >}}).
...
# CadenzaFlow Architecture

CadenzaFlow is a flexible framework which can be deployed in different scenarios. This section provides an overview of the most common deployment scenarios.
...
CadenzaFlow doesn't provide load-balancing capabilities or session replication capabilities out of the box. The load-balancing function would need to be provided by a third-party system, and session replication would need to be provided by the host application server.
...
If a cookie based consistent hashing is used, make sure that the cookie name is not JSESSIONID which is used by CadenzaFlow.
{{< /note >}}
...
To serve multiple, independent parties with one CadenzaFlow installation, the process engine supports
...
Users should choose the model which fits their data separation needs. CadenzaFlow's APIs provide access
...
The CadenzaFlow web applications are based on a RESTful architecture.
...
Additional custom frameworks developed by CadenzaFlow hackers:

* [cadenzaflow-bpmn.js](https://github.com/cadenzaflow/cadenzaflow-bpmn.js): CadenzaFlow BPMN 2.0 JavaScript libraries
