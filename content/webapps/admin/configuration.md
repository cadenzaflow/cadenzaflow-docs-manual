...
If you connect CadenzaFlow with the LDAP identity service, you have read-only access to the users and groups. Create new users and groups via the LDAP system, but not in the admin application. Find more information about how to configure the process engine in order to use the LDAP identity service [here]({{< ref "/user-guide/process-engine/identity-service.md#the-ldap-identity-service" >}}).
...
```css
.navbar-brand {
  /* hides the "CadenzaFlow Admin" text */
  text-indent: -999em;
  /* put your logo */
...
**Note:** you can also change the app name (*Admin*) and vendor (*CadenzaFlow*)
...
Admin can be localized. CadenzaFlow maintains English and German translation files. 
...
```
This includes a `custom-module/module.js` file. The path is relative to the `app/admin` folder in the CadenzaFlow webapp .war file.
...
at the variables defined in the following files:
 ...
 - `node_modules/cadenzaflow-commons-ui/node_modules/bootstrap/less/variables.less`
   defines the original Bootstrap variables
 - `node_modules/cadenzaflow-commons-ui/resources/less/cam-variables.less`
   overrides some Bootstrap variables (above) and add some custom ones

## Compiling with Grunt

From within the `cadenzaflow-bpm-webapp` directory:
