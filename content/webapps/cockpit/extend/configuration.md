...
```css
/* hides the CadenzaFlow logo */
.app-banner svg {
  display: none;
}
.app-banner {
  /* hides the "CadenzaFlow Cockpit" text */
  text-indent: 200vw;
  /* put your logo */
...
**Note:** you can also change the app name (*Cockpit*) and vendor (*CadenzaFlow*)
...
Cockpit can be localized. CadenzaFlow maintains English and German translation files. 
You can find and download community maintained translation files at the [CadenzaFlow webapp translations repository](https://github.com/cadenzaflow/cadenzaflow-webapp-translations).
...
```
This includes a `custom-module/module.js` file. The path is relative to the `app/cockpit` folder in the CadenzaFlow webapp .war file.

You can find a complete example about how to use `customScripts` to develop a Cockpit Plugin in the [CadenzaFlow examples repository](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/cockpit/cockpit-cats).

## Legacy Custom Scripts

Custom Scripts created for CadenzaFlow.13 or earlier can be included using the `requireJsConfig` property to the `app/cockpit/scripts/config.js`. You can include these custom scripts using the custom [requireJS configuration](https://requirejs.org/docs/api.html#config).
...
For more details about legacy Plugins inherited from Camunda, check out the legacy [Camunda Plugin documentation](https://docs.camunda.org/manual/7.13/webapps/cockpit/extend/plugins/). Please note that this link will take you to the documentation of Camunda **7.13** .
...
## Additional Modules
To add modules, the `additionalModules` property needs to be specified, where each module is registered with its path. The path is relative to the `app/cockpit` folder in the .war file of the CadenzaFlow Webapp.
...
You can find an example on how to add an additional bpmn.js module to Cockpit in the [CadenzaFlow examples repository](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/cockpit/cockpit-bpmn-js-module).
...
.war file of the CadenzaFlow Webapp. The suffix `.json` of the file is added automatically and must not be specified.
```json
export default {
  // …
  bpmnJs: {
    moddleExtensions: {
      cadenzaflow: 'my-custom-moddle/cadenzaflow'
    }
  }
}
```
...
First-time visitors are shown a message directing them to the cadenzaflow welcome page. If you do
