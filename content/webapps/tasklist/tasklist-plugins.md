...
{{< note title="Plugin Compatibility" class="info" >}}
  Please note that the code of Tasklist plugins might need to be migrated when updating CadenzaFlow to a higher version (e.g. CSS styles).
{{< /note >}}
...
{{< note title="Difference between Cockpit and Tasklist plugins:" class="warning">}}
  * To publish the plugin with Tasklist, its class name must be put into a file called ```org.cadenzaflow.bpm.tasklist.plugin.spi.TasklistPlugin``` that resides in the directory ```META-INF/services```.
...
* [How to build the server side](https://github.com/cadenzaflow/cadenzaflow-bpm-platform/tree/master/webapps/assembly/src/main/java/org/cadenzaflow/bpm/tasklist/impl/plugin)
* [How to build the client side](https://github.com/cadenzaflow/cadenzaflow-bpm-platform/tree/master/webapps/frontend/ui/tasklist/plugins/standaloneTask/app)
