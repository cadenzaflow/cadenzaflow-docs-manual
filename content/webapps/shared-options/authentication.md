...
Authentication means verifying a user's identity against the CadenzaFlow web apps. 
...
The CadenzaFlow web apps correlate the authentication information against [authorizations]({{< ref "/user-guide/process-engine/authorization-service.md" >}}) to determine 
...
<filter>
  ...
  <filter-class>org.cadenzaflow.bpm.webapp.impl.security.auth.AuthenticationFilter</filter-class>
  <init-param>
...
CadenzaFlow supports a broad range of containers, including Tomcat and Wildfly. Using Container-Based Authentication, it is possible to move the authentication action to the container level, which will then make the authentication result available to the CadenzaFlow Web Applications.
...
```xml
  ...
  <filter>
    ...
    <filter-class>org.cadenzaflow.bpm.webapp.impl.security.auth.ContainerBasedAuthenticationFilter</filter-class>
    <init-param>
      <param-name>authentication-provider</param-name>
      <param-value>org.cadenzaflow.bpm.engine.rest.security.auth.impl.ContainerBasedAuthenticationProvider</param-value>
    </init-param>
  </filter>
  <filter-mapping>
    <filter-name>Container Based Authentication Filter</filter-name>
...
The CadenzaFlow Web Applications can also integrate with a [Single Sign-On implementation](https://en.wikipedia.org/wiki/List_of_single_sign-on_implementations) when the Container-Based Authentication servlet filter is enabled.
