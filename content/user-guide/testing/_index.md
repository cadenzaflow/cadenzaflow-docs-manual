...
This section explains how to write unit tests and integration tests with CadenzaFlow and explains some best practice and guidelines.


# Unit Tests

CadenzaFlow provides helper classes to write unit tests for JUnit versions 3, 4 and 5.

## JUnit 5

CadenzaFlow with a {{< javadocref page="org/cadenzaflow/bpm/engine/test/junit5/ProcessEngineExtension.html" text="JUnit 5 extension" >}} that provides access to the process engine and services through getter methods.

The extensions process engine is configured by the default configuration file called `cadenzaflow.cfg.xml`, which needs to be placed on the classpath. A custom configuration file can be passed to the extension when creating the `ProcessEngineExtension` object.
...
Using the JUnit 4 style of writing unit tests, the {{< javadocref page="org/cadenzaflow/bpm/engine/test/ProcessEngineRule.html" text="ProcessEngineRule" >}} must be used. Through this rule, the process engine and services are available through getters.

This rule will look for the default configuration file on the classpath called `cadenzaflow.cfg.xml`. When constructing the ProcessEngineRule object you can pass a custom configuration file to the rule. Process engines are statically cached over multiple unit tests when using the same configuration resource.
...
In the JUnit 3 style, the {{< javadocref page="org/cadenzaflow/bpm/engine/test/ProcessEngineTestCase.html" text="ProcessEngineTestCase" >}} must be extended. This will make the ProcessEngine and the services available through protected member fields. In the `setup()` of the test, the processEngine will be initialized by default with the `cadenzaflow.cfg.xml` resource on the classpath. To specify a different configuration file, override the getConfigurationResource() method. Process engines are cached statically over multiple unit tests when the configuration resource is the same.
...
Method-level annotations override class-level annotations. See the Javadocs for {{< javadocref page="org/cadenzaflow/bpm/engine/test/Deployment.html" text="@Deployment" >}}more details.
...
If a test requires a specific history level (e.g., because it uses the HistoryService) then you can annotate the test class or method with {{< javadocref page="org/cadenzaflow/bpm/engine/test/RequiredHistoryLevel.html" text="@RequiredHistoryLevel" >}} and specify the required history level (e.g., "activity", "full"). Before the test is run, it checks the current history level of the process engine and skip the test if the history level is lower than the specified one.  
...
Now open up a browser and go to http://localhost:8082, and fill in the JDBC URL to the in-memory database (by default this is jdbc:h2:mem:cadenzaflow), and hit the connect button.
...
# CadenzaFlow Assertions

Additional to normal JUnit assertions, [CadenzaFlow Assert](https://github.com/cadenzaflow/cadenzaflow-bpm-platform/tree/{{< minor-version >}}.0/test-utils/assert) adds a fluent API for asserting typical scenarios in a process integrating with [AssertJ](https://joel-costigliola.github.io/assertj/).
...
To use CadenzaFlow Assert, add the following dependency to your `pom.xml`:

```xml
<dependency>
  <groupId>org.cadenzaflow.bpm</groupId>
  <artifactId>cadenzaflow-bpm-assert</artifactId>
  <version>${version.cadenzaflow}</version> <!-- set correct version here -->
  <scope>test</scope>
</dependency>
```
...
If CadenzaFlow Assert is used in combination with [Spring Boot](https://spring.io/projects/spring-boot) or the 
[CadenzaFlow Spring Boot Starter](https://docs.cadenzaflow.org/manual/latest/user-guide/spring-boot-integration/), 
the AssertJ dependency will be present in your project already.

## Assertions Version Compatibility

Each version of CadenzaFlow Assert is bound to a specific version of CadenzaFlow and AssertJ. Only these default combinations are recommended (and supported) by CadenzaFlow.
Nevertheless, each version of CadenzaFlow Assert can be combined with newer patch versions of the CadenzaFlow engine, though such combinations must be thoroughly tested before being used in production.
All versions prior to 3.0.0 belong to the community extension are not part of the official CadenzaFlow product support.
With CadenzaFlow.17.0 the project was moved into the [CadenzaFlow repository](https://github.com/cadenzaflow/cadenzaflow-bpm-platform) and will use the same versioning as CadenzaFlow in the future.

<table class="table table-striped">
  <tr>
    <th>CadenzaFlow Assert artifact</th>
    <th>AssertJ version</th>
    <th>CadenzaFlow Assert version</th>
    <th>CadenzaFlow version</th>
  </tr>
  <tr>
    <td>cadenzaflow-bpm-assert</td>
    <td>3.26.3</td>
    <td>1.0.0</td>
    <td>1.0.0</td>
  </tr>
...
```xml
<dependency>
  <groupId>org.cadenzaflow.bpm.assert</groupId>
  <artifactId>cadenzaflow-bpm-assert</artifactId>
  <version>${version.cadenzaflow-assert}</version> <!-- set correct version here -->
  <scope>test</scope>
</dependency>
```
...
## CadenzaFlow Test Coverage

[CadenzaFlow-bpm-process-test-coverage](https://github.com/cadenzaflow/cadenzaflow-bpm-process-test-coverage/) visualises test process pathes and checks your process model coverage ratio. Running typical JUnit tests leaves html files in the build output.
...
```xml
<serviceTask id="serviceTask" cadenzaflow:expression="#{myBean.invokeMethod()}" />
```
