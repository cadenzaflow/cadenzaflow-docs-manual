...
processes in a few lines of code. In the [generate process fluent api](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/bpmn-model-api/generate-process-fluent-api) quickstart we
...
BpmnModelInstance modelInstance = Bpmn.createProcess()
  ...
  .intermediateThrowEvent("throw")
    .signalEventDefinition("signal")
      .cadenzaflowInSourceTarget("source", "target1")
      .cadenzaflowInSourceExpressionTarget("${'sourceExpression'}", "target2")
      .cadenzaflowInAllVariables("all", true)
      .cadenzaflowInBusinessKey("aBusinessKey")
...
The transaction boundaries of a process created with the fluent builder API can be controlled using the `cadenzaflowAsyncBefore()` and `cadenzaflowAsyncAfter()` methods offered for various process constructs.

```java
BpmnModelInstance modelInstance = Bpmn.createProcess()
  .startEvent()
  .serviceTask("servicetask")
    .cadenzaflowAsyncBefore()
  .userTask("task")
    .cadenzaflowAsyncAfter()
  .done();
```
...
If an activity has [multi-instance characteristics][multi-instance], the  `cadenzaflowAsyncBefore()` and `cadenzaflowAsyncAfter()` methods apply to the multi-instance body as a whole. The transaction boundaries of the individual occurrences (instances) of the multi-instance can be controlled with similar methods, called from **within** the multi-instance builder.

```java
BpmnModelInstance modelInstance = Bpmn.createProcess()
  .startEvent()
  .serviceTask("servicetask")
    .cadenzaflowAsyncBefore() // multi-instance body
    .multiInstance()
      .cadenzaflowAsyncBefore() // every instance
      .parallel()
    .multiInstanceDone()
  .endEvent()
  .done();
