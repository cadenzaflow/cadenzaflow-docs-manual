...
The [CadenzaFlow extension elements]({{< ref "/reference/bpmn20/custom-extensions/extension-elements.md" >}}) are fully implemented in the BPMN model API, but unknown extension elements can also easily be accessed and added.
...
```java
CadenzaflowFormData formData = modelInstance.newInstance(CadenzaflowFormData.class);
...
extensionElements.getElementsQuery().filterByType(CadenzaflowFormData.class).singleResult();
```
...
you can use the `class` of a known extension element type, e.g., the cadenzaflow
...
CadenzaflowExecutionListener listener = extensionElements.addExtensionElement(CadenzaflowExecutionListener.class);
```

Another helper method exists for the fluent builder API which allows you to add prior defined extension elements.

```java
CadenzaflowExecutionListener cadenzaflowExecutionListener = modelInstance.newInstance(CadenzaflowExecutionListener.class);
cadenzaflowExecutionListener.setCadenzaflowClass("org.cadenzaflow.bpm.MyJavaDelegte");
startEvent.builder()
  .addExtensionElement(cadenzaflowExecutionListener);
```
