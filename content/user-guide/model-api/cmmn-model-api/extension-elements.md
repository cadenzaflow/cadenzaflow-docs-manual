...
The [CadenzaFlow extension elements]({{< ref "/reference/cmmn11/custom-extensions/cadenzaflow-elements.md" >}}) are fully implemented in the CMMN model API but unknown extension elements can also easily be accessed and added.
...
```java
CadenzaflowCaseExecutionListener listener = modelInstance.newInstance(CadenzaflowCaseExecutionListener.class);
...
extensionElements.getElementsQuery().filterByType(CadenzaflowCaseExecutionListener.class).singleResult();
```
...
CadenzaflowCaseExecutionListener listener = extensionElements.addExtensionElement(CadenzaflowCaseExecutionListener.class);
```
