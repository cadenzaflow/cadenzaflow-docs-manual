...
The auto starter uses the  `org.cadenzaflow.bpm.engine.impl.cfg.ProcessEnginePlugin` mechanism to configure the engine.

The configuration is divided into _sections_. These _sections_ are represented by the marker interfaces:

* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowProcessEngineConfiguration`
* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowDatasourceConfiguration`
* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowHistoryConfiguration`
* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowHistoryLevelAutoHandlingConfiguration`
* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowJobConfiguration`
* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowDeploymentConfiguration`
* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowAuthorizationConfiguration`
* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowFailedJobConfiguration`
* `org.cadenzaflow.bpm.spring.boot.starter.configuration.CadenzaflowMetricsConfiguration`
...
Configures the CadenzaFlow data source and enables [transaction integration]({{< ref "/user-guide/spring-framework-integration/transactions.md" >}}). By default, the primary `DataSource` and `PlatformTransactionManager` beans are wired with the process engine configuration.
...
data source with name `cadenzaflowBpmDataSource` that will be automatically wired with CadenzaFlow instead.
...
@Bean(name="cadenzaflowBpmDataSource")
...
transaction manager with the name `cadenzaflowBpmTransactionManager` that will be wired with the data
source used for CadenzaFlow (either `@Primary` or `cadenzaflowBpmDataSource`):
...
@Bean(name="cadenzaflowBpmTransactionManager")
public PlatformTransactionManager cadenzaflowTransactionManager(@Qualifier("cadenzaflowBpmDataSource") DataSource dataSource) {
  return new DataSourceTransactionManager(dataSource);
}
```

{{< note title="" class="warning" >}}
  The wired data source and transaction manager beans must match, i.e. make sure that the transaction manager actually manages the CadenzaFlow data source. If that is not the case, the process engine will use auto-commit mode for the data source connection, potentially leading to inconsistencies in the database.
{{< /note >}}
...
As cadenzaflow version >= 7.4 supports `history-level auto`, this configuration adds support for versions <= 7.3.

To have more control over the handling, you can provide your own

- `org.cadenzaflow.bpm.spring.boot.starter.jdbc.HistoryLevelDeterminator` with name `historyLevelDeterminator`
...
- `org.cadenzaflow.bpm.engine.impl.jobexecutor.JobExecutor`
- `org.springframework.core.task.TaskExecutor` named `cadenzaflowTaskExecutor`

beans.

IMPORTANT: The job executor is not enabled in the configuration.
This is done after the spring context successfully loaded (see `org.cadenzaflow.bpm.spring.boot.starter.runlistener`).

### `DefaultDeploymentConfiguration`

If auto deployment is enabled (this is the case by default), all processes found in the classpath are deployed.
The resource pattern can be changed using properties (see [properties](#cadenzaflow-engine-properties)).

### `DefaultAuthorizationConfiguration`

Applies the authorization configuration to the process engine. If not configured, the `cadenzaflow` default values are used (see [properties](#cadenzaflow-engine-properties)).
...
```java
@Configuration
public class MyCadenzaflowConfiguration {

	@Bean
	public static CadenzaflowDatasourceConfiguration cadenzaflowDatasourceConfiguration() {
		return new MyCadenzaflowDatasourceConfiguration();
	}
...
You just have to provide one or more beans implementing the `org.cadenzaflow.bpm.engine.impl.cfg.ProcessEnginePlugin` interface
(or extend from `org.cadenzaflow.bpm.spring.boot.starter.configuration.impl.AbstractCadenzaflowConfiguration`).
...
@Component
@Order(Ordering.DEFAULT_ORDER + 1)
public class MyCustomConfiguration extends AbstractCadenzaflowConfiguration {
...
can be found in the <a href="https://docs.cadenzaflow.org/get-started/spring-boot/configuration/">Spring Boot Starter Guide</a>.
...
<tr><td rowspan="15"><code>cadenzaflow.bpm</code></td>
<td><code>.enabled</code></td>
<td>Switch to disable the CadenzaFlow auto-configuration. Use to exclude CadenzaFlow in integration tests.</td>
...
<td>CadenzaFlow default value</td>
</tr>
...
<td>CadenzaFlow history level</td>
<td>FULL</td>
</tr>

<tr>
<td><code>.history-level-default</code></td>
<td>CadenzaFlow history level to use when <code>history-level</code> is <code>auto</code>, but the level can not determined automatically</td>
<td>FULL</td>
</tr>
...
<td>Provides a URL to your CadenzaFlow license file and is automatically inserted into the DB when the application starts (but only if no valid license key is found in the DB).</br></br>
<b>Note:</b> This property is only available when using the <a href="{{<ref "/user-guide/spring-boot-integration/webapps.md#enterprise-webapps" >}}">cadenzaflow-bpm-spring-boot-starter-webapp-ee</a>
</td>
<td>By default, the license key will be loaded:
 <ol>
  <li>from the URL provided via the this property (if present)</li>
  <li>from the file with the name <code>cadenzaflow-license.txt</code> from the classpath (if present)</li>
  <li>from path <i>${user.home}/.cadenzaflow/license.txt</i> (if present)</li>
 </ol>
...
<tr>
<td rowspan="14"><code>cadenzaflow.bpm.job-execution</code></td>
<td><code>.enabled</code></td>
...
<tr>
<td rowspan="5"><code>cadenzaflow.bpm.database</code></td>
...
<td>Prefix of the cadenzaflow database tables. Attention: The table prefix will <b>not</b> be applied if you  are using <code>schema-update</code>!</td>
<td><i>CadenzaFlow default value</i></td>
</tr>
...
<td><i>CadenzaFlow default value: true</i></td>
</tr>

<tr><td colspan="4"><b>Eventing</b></td></tr>
<tr>
<td rowspan="4"><code>cadenzaflow.bpm.eventing</code></td>
...
<tr>
<td><code>cadenzaflow.bpm.management</code></td>
<td><code>.health.cadenzaflow.enabled</code></td>
<td>Enables default cadenzaflow health indicators</td>
<td><code>true</code></td>
</tr>

<tr><td colspan="4"><b>Metrics</b></td></tr>
<tr>
<td rowspan="2"><code>cadenzaflow.bpm.metrics</code></td>
<td><code>.enabled</code></td>
...
<tr>
<td rowspan="3"><code>cadenzaflow.bpm.webapp</code></td>
<td><code>.enabled</code></td>
<td>Switch to disable the CadenzaFlow Webapp auto-configuration.</td>
<td><code>true</code></td>
</tr>

<tr>
<td><code>.index-redirect-enabled</code></td>
<td>Registers a redirect from <code>/</code> to cadenzaflow's bundled <code>index.html</code>.
<br/>
...
<br/>
When setting to <code>/</code>, the legacy behavior of CadenzaFlow Spring Boot Starter <= 3.4.x is restored.
</td>
<td><code>/cadenzaflow</code></td>
</tr>

<tr id="csrf">
  <td rowspan="10"><code>cadenzaflow.bpm.webapp.csrf</code></td>
</tr>
<tr>
...
<tr id="session-cookie">
  <td rowspan="6"><code>cadenzaflow.bpm.webapp.session-cookie</code></td>
</tr>
<tr>
  <td><code>.enable-secure-cookie</code></td>
  <td>
...
<tr id="header-security">
  <td rowspan="12"><code>cadenzaflow.bpm.webapp.header-security</code></td>
</tr>
<tr>
...
<tr id="auth-cache">
  <td rowspan="3"><code>cadenzaflow.bpm.webapp.auth.cache</code></td>
</tr>
<tr>
...
<tr>
<td rowspan="4"><code>cadenzaflow.bpm.authorization</code></td>
<td><code>.enabled</code></td>
...
<tr>
<td rowspan="3"><code>cadenzaflow.bpm.admin-user</code></td>
<td><code>.id</code></td>
...
<tr>
<td><code>cadenzaflow.bpm.filter</code></td>
...
<tr>
  <td rowspan="3"><code>cadenzaflow.bpm.oauth2.identity-provider</code></td>
  <td><code>.enabled</code></td>
...
<tr>
  <td rowspan="2"><code>cadenzaflow.bpm.oauth2.sso-logout</code></td>
  <td><code>.enabled</code></td>
...
CadenzaFlow Spring Boot Starter versions <= 2.3 (Spring Boot version 1.x)
...
CadenzaFlow Spring Boot Starter versions >= 3.0 (Spring Boot version 2.x)
...
[cadenzaflow.bpm.webapp.session-cookie]({{< ref "/user-guide/spring-boot-integration/configuration.md#session-cookie" >}}) in the `application.yaml`.

# Configuring Spin DataFormats

The CadenzaFlow Spring Boot Starter auto-configures the Spin Jackson Json DataFormat when the
`cadenzaflow-spin-dataformat-json-jackson` dependency is detected on the classpath. To include a
`DataFormatConfigurator` for the desired Jackson Java 8 module, the appropriate dependency needs
to be included on the classpath as well. Note that `cadenzaflow-engine-plugin-spin`
...
`cadenzaflow-spin-dataformat-all` as a dependency. The `cadenzaflow-spin-dataformat-all` artifact shades the
Jackson libraries, which breaks compatibility with the regular Jackson modules. If usage of
`cadenzaflow-spin-dataformat-all` is necessary, please use the standard method for
...
<dependencies>
    <dependency>
      ...
      <artifactId>cadenzaflow-engine-plugin-spin</artifactId>
    </dependency>
    <dependency>
      <groupId>org.cadenzaflow.spin</groupId>
      <artifactId>cadenzaflow-spin-dataformat-json-jackson</artifactId>
    </dependency>
    <dependency>
...
1. Provide a custom implementation of `org.cadenzaflow.spin.spi.DataFormatConfigurator`;
