...
CadenzaFlow provides Spring Security OAuth2 integration with the `cadenzaflow-bpm-spring-boot-starter-security` library.
This library contains the Spring Security and Spring Security OAuth2 dependencies along with
configuration classes that integrate Spring Security with CadenzaFlow Webapp's authentication.

This is available both for Spring Boot and cadenzaflow Run.

CadenzaFlow's integration comes with multiple components and configurations. In the next sections you can find more details to each of them.
...
<dependency>
  ...
  <artifactId>cadenzaflow-bpm-spring-boot-starter-security</artifactId>
</dependency>
```

## CadenzaFlow Run

CadenzaFlow Run already contains the required libraries, all you need to do is to activate them.
In order to enable the Spring Security OAuth2 integration in CadenzaFlow Run, start Run with an extra `--oauth2` argument:
...
The CadenzaFlow integration has two default auto configurations. Depending on the OAuth2 client
registration in the application properties (`spring.security.oauth2.client.registration`) either the
`CadenzaflowSpringSecurityOAuth2AutoConfiguration` or the `CadenzaflowBpmSpringSecurityDisableAutoConfiguration` will be activated.
...
Security filter chain to secure the CadenzaFlow Webapps.

Spring auto configuration class: {{< javadocref page="org/cadenzaflow/bpm/spring/boot/starter/security/oauth2/impl/CadenzaflowSpringSecurityOAuth2AutoConfiguration.html" text="CadenzaflowSpringSecurityOAuth2AutoConfiguration" >}}
...
Spring auto configuration class: {{< javadocref page="org/cadenzaflow/bpm/spring/boot/starter/security/oauth2/impl/CadenzaflowBpmSpringSecurityDisableAutoConfiguration.html" text="CadenzaflowBpmSpringSecurityDisableAutoConfiguration" >}}
...
Once there is an OAuth2 client registration configured and the CadenzaFlow Spring Security OAuth2
integration is enabled, the Webapps will use the configured OAuth2 provider for
authentication.

# User Name Mapping

CadenzaFlow's integration uses the **name** field from Spring Security's principal object as the User ID
in the Webapps.

Spring Security by default uses the subject (`sub`) claim as the principal name. As the User ID in CadenzaFlow is
...
1. The `cadenzaflow.bpm.admin-user` property to create an administrator user with authorizations:
   ```yaml
   cadenzaflow.bpm:
     admin-user:
       id: demo
  ...
  - See [CadenzaFlow Engine Properties]({{< ref "/user-guide/spring-boot-integration/configuration#cadenzaflow-engine-properties" >}}) documentation for more details.
...
Additionally to the OAuth2 login, CadenzaFlow also provides support to use groups from OAuth2.
This is achieved with a custom [identity service]({{< ref "/user-guide/process-engine/identity-service.md" >}}), called {{< javadocref page="org/cadenzaflow/bpm/spring/boot/starter/security/oauth2/impl/OAuth2IdentityProvider.html" text="OAuth2IdentityProvider" >}}.
...
This identity provider also supports the default CadenzaFlow Database Identity Service as a fallback for authentications for the REST API.

The identity provider is activated by default. You can override this with the following properties:
```yaml
cadenzaflow.bpm.oauth2:
  identity-provider:
    enabled: false
```
...
In CadenzaFlow Run a JAR file needs to be built and copied into the `userlib` folder.
This needs to contain a [Spring auto configuration][AutoConfig] with the custom granted authorities mapper bean.

## Configuration

All configuration properties of the identity provider start with the prefix `cadenzaflow.bpm.oauth2.identity-provider`.
...
Furthermore, it only shows groups from OAuth2 and doesn't show groups configured in CadenzaFlow database.
...
be also configured with the matching User ID in CadenzaFlow database.
...
In order to support both logouts, the CadenzaFlow integration also contains a Frontend Plugin that overrides the Webapps default logout behaviour.
As a consequence, when the Webapp user clicks on the logout, it invokes Spring's logout endpoint (`/logout`) instead of CadenzaFlow's.
...
cadenzaflow.bpm.oauth2:
  sso-logout:
    enabled: true
    postLogoutRedirectUri: https://cadenzaflow.com/
```

## Configuration

All configuration properties of the identity provider start with the prefix `cadenzaflow.bpm.oauth2.sso-logout`.
The following properties are available:
...
CadenzaFlow's integration heavily relies on Spring Security's OAuth2 support.

If you decide to use OAuth2 for login in CadenzaFlow, we highly recommend to consult and implement the current industry recommended security standards.
...
logging:
  level:
    org.cadenzaflow.bpm.spring.boot.starter.security.oauth2: DEBUG
```
...
4. Defines the `openid,profile,email,offline_access` scopes.
   ...
   - In case of OKTA, `profile` and `email` are useful to access firstname, lastname and email in CadenzaFlow but not mandatory.
   - `offline_access` activates the refresh_token grant, not mandatory.
5. Configures the `preferred_username` as the username attribute, which is also used as the CadenzaFlow User ID.

# Disable Auto Configuration

If you wish to use Spring Security but without CadenzaFlow's integration classes, you can do so by
...
@EnableAutoConfiguration(exclude={
    CadenzaflowSpringSecurityOAuth2AutoConfiguration.class,
    CadenzaflowBpmSpringSecurityDisableAutoConfiguration.class
});
```
...
spring:
  autoconfigure:
    exclude:
      - org.cadenzaflow.bpm.spring.boot.starter.security.oauth2.CadenzaflowSpringSecurityOAuth2AutoConfiguration
      - org.cadenzaflow.bpm.spring.boot.starter.security.oauth2.CadenzaflowBpmSpringSecurityDisableAutoConfiguration
```
