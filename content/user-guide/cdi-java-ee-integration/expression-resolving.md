...
The `cadenzaflow-engine-cdi` and `cadenzaflow-engine-cdi-jakarta` libraries expose CDI beans via Expression Language, 
using a custom resolver. This makes it possible to reference beans from the process:

```xml
<userTask id="authorizeBusinessTrip" name="Authorize Business Trip"
                        cadenzaflow:assignee="#{authorizingManager.account.username}" />
```
...
We can use the same feature to call a business method of an EJB in a service task, using the `cadenzaflow:expression="${myEjb.method()}"`-extension.
