...
In this section we briefly look at the contextual process execution model used by the CadenzaFlow CDI extension.
...
In CadenzaFlow CDI, a process instance can be associated with a CDI scope, the association representing a unit of work.
...
The CadenzaFlow CDI integration provides the `org.cadenzaflow.bpm.engine.cdi.BusinessProcess` bean for controlling the association, most prominently:
...
CadenzaFlow CDI allows declaratively starting process instances and completing tasks using annotations. 
The `@org.cadenzaflow.bpm.engine.cdi.annotation.StartProcess` annotation allows to start a process instance either by "key" or by "name". 
...
Depending on the configuration of the CadenzaFlow engine, the code of the annotated method and the starting of the process
instance will be combined in the same transaction. The `@org.cadenzaflow.bpm.engine.cdi.annotation.CompleteTask`-annotation
...
Using CadenzaFlow CDI, the lifecycle of a bean can be bound to a process instance. To this extent, a custom context implementation is provided,
