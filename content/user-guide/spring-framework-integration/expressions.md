...
<bean id="printer"
      class="org.cadenzaflow.bpm.engine.spring.test.transaction.Printer" />
```
...
<definitions id="definitions" ...>
  ...
  <process id="helloProcess" isExecutable="true">
    ...
    <serviceTask id="print" 
                 cadenzaflow:expression="#{printer.printMessage(execution)}" />
...
This functionality is provided by the `org.cadenzaflow.bpm.engine.spring.application.SpringProcessApplicationElResolver`. This class is a `ProcessApplicationElResolver` implementation delegating to the local application context. Expression resolving then works in the following way:
...
{{< note title="" class="info" >}}
  The `SpringProcessApplicationElResolver` class is automatically detected if the `cadenzaflow-engine-spring` module is included as a library of the process application, not as a global library.
{{< /note >}}
...
<beans ...>
  ...
  <bean id="testbean" 
        class="org.cadenzaflow.bpm.engine.spring.test.scripttask.Testbean" />
</beans>
```
