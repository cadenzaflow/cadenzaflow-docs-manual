...
This section of the CadenzaFlow Quarkus extension documentation covers the configuration options for the process engine
within a Quarkus application.

The documentation on the CadenzaFlow Quarkus Extension Configuration is intended for Quarkus application developers. It 
requires some knowledge on [Quarkus CDI support][quarkus-cdi], [Quarkus configuration][quarkus-config], as well as
CadenzaFlow Process Engine Configuration properties.
...
<table class="table desc-table">
  ...
  <tr>
    ...
    <td>
      An instance of {{< javadocref page="org/cadenzaflow/bpm/engine/impl/persistence/StrongUuidGenerator.html" text="StrongUuidGenerator" >}}
      is used.
    </td>
  ...
  <tr>
    ...
    <td>
      CadenzaFlow Cockpit works best with history level 
...
configuring a Quarkus application in the [Quarkus configuration][quarkus-config] page. The CadenzaFlow Quarkus extension 
docs use the `application.properties` format in the examples, but you can use any supported Quarkus config source.

You can set any process engine configuration properties under the `quarkus.cadenzaflow` prefix. The 
...
```properties
quarkus.cadenzaflow.generic-config.cmmn-enabled=false
quarkus.cadenzaflow.generic-config.dmn-enabled=false
quarkus.cadenzaflow.generic-config.history=none
```
...
```properties
quarkus.cadenzaflow.generic-config.process-engine-name=quarkusEngine
```
...
configuration properties under the `quarkus.cadenzaflow.job-executor` prefix. The [Job Executor Configuration Properties][executor-properties] 
...
```properties
quarkus.cadenzaflow.job-executor.generic-config.max-jobs-per-acquisition=5
quarkus.cadenzaflow.job-executor.generic-config.lock-time-in-millis=500000
quarkus.cadenzaflow.job-executor.generic-config.wait-time-in-millis=7000
quarkus.cadenzaflow.job-executor.generic-config.max-wait=65000
```
...
sections, the CadenzaFlow Quarkus extension provides some Quarkus-specific configuration properties. They can be set
...
<table class="table desc-table">
  ...
  <tr>
    <td rowspan="1"><code>quarkus.cadenzaflow</code></td>
  ...
  <tr>
    <td rowspan="2"><code>quarkus.cadenzaflow.job-executor.thread-pool</code></td>
...
A datasource is required to run the CadenzaFlow process engine.
...
should use by its name via the `cadenzaflow.datasource` configuration property. Consider the example configuration below:
...
quarkus.cadenzaflow.datasource=engine-datasource
```
...
# job executor configuration
quarkus.cadenzaflow.job-executor.thread-pool.max-pool-size=12
quarkus.cadenzaflow.job-executor.thread-pool.queue-size=5
quarkus.cadenzaflow.job-executor.generic-config.max-jobs-per-acquisition=5
quarkus.cadenzaflow.job-executor.generic-config.lock-time-in-millis=500000
quarkus.cadenzaflow.job-executor.generic-config.wait-time-in-millis=7000
quarkus.cadenzaflow.job-executor.generic-config.max-wait=65000
quarkus.cadenzaflow.job-executor.generic-config.backoff-time-in-millis=5

# custom data source configuration and selection
quarkus.datasource.my-datasource.db-kind=h2
quarkus.datasource.my-datasource.username=cadenzaflow
quarkus.datasource.my-datasource.password=cadenzaflow
quarkus.datasource.my-datasource.jdbc.url=jdbc:h2:mem:cadenzaflow;TRACE_LEVEL_FILE=0;DB_CLOSE_ON_EXIT=FALSE
quarkus.cadenzaflow.datasource=my-datasource
```
