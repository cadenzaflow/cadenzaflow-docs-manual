...
process engine. The documentation assumes some familiarity with [Quarkus CDI support][quarkus-cdi] and the CadenzaFlow 
{{< javadocref page="org/cadenzaflow/bpm/engine/repository/DeploymentBuilder.html" text="DeploymentBuilder" >}} API.

The CadenzaFlow Engine Quarkus Extension only supports programmatic deployments. A user can observe for the
`CadenzaflowEngineStartupEvent` CDI event. The `CadenzaflowEngineStartupEvent` signals that a process engine has been
...
public class MyConfig {
  ...
  public void createDeployment(@Observes CadenzaFlowEngineStartupEvent event) {
...
the `CadenzaFlowEngineStartupEvent` in multiple methods, and perform multiple deployments with a finer-grained control
...
public class MyConfig {
  ...
  public void createDeployment1(@Observes CadenzaFlowEngineStartupEvent event) {
    repositoryService.createDeployment()
        .name("deployment-1")
  ...
  public void createDeployment2(@Observes CadenzaFlowEngineStartupEvent event) {
    repositoryService.createDeployment()
        .name("deployment-2")
