...
The CadenzaFlow Engine offers CDI 4 integration with the `cadenzaflow-engine-cdi-jakarta` module. This module is integrated 
...
the `cadenzaflow-engine-cdi-jakarta` module provides. Some features documented under 
...
that affect the functionality of the `cadenzaflow-engine-cdi-jakarta` module. For your individual application development, 
...
The `cadenzaflow-engine-cdi-jakarta` module allows referencing CDI beans and calling methods on CDI beans in 
model expression properties (e.g., `cadenzaflow:expression`, `cadenzaflow:delegateExpression`, etc.). 
...
While the `cadenzaflow-engine-cdi-jakarta` module supports associating a process instance with 
...
The `cadenzaflow-engine-cdi-jakarta` module allows setting variables when assigning a value to a class field
...
The `cadenzaflow-engine-cdi-jakarta` module stores [`@BusinessProcessScoped`][business-process-scoped] beans as 
...
* `org.cadenzaflow.bpm.engine.cdi.jsf.TaskForm`
* `org.cadenzaflow.bpm.engine.cdi.compat.FoxTaskForm`
* `org.cadenzaflow.bpm.engine.cdi.compat.CadenzaFlowTaskForm`
...
[destroy-jira-issue]: https://jira.cadenzaflow.com/browse/CAM-13755
