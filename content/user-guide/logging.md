...
This page provides information about logging in CadenzaFlow.

# SLF4J

Most CadenzaFlow modules, including the CadenzaFlow engine, use [slf4j] as logging "facade". This allows users to direct logging output to the logging "backend" of their choice, such as [logback] or [log4j].

## Preconfigured Logging with a Shared Process Engine

When installing CadenzaFlow as a shared process engine in an application server, CadenzaFlow logging is pre-configured.

On application servers, logging is pre-configured using the slf4j-jdk14 bridge.
This means that CadenzaFlow effectively re-directs all its logging to Java Util Logging.
...
When using the CadenzaFlow Maven modules in a custom application, only the [slf4j] API is pulled in transitively.
...
<table class="table table-striped">
  ...
  <tr>
    <td><code>org.cadenzaflow.bpm.webapp</code></td>
    <td>logs events from the CadenzaFlow web apps (Cockpit, Tasklist, and Admin), like user-initiated log in and log out events and cache validation time information.</td>
  </tr>
</table>
...
The output of loggers can change with newer CadenzaFlow versions.
{{< /note >}}
...
To check the database statements performed by the engine in most of the cases, it will be sufficient to increase the level of `org.cadenzaflow.bpm.engine.persistence` and `org.cadenzaflow.bpm.engine.impl.persistence.entity` loggers.
...
* `org.cadenzaflow.bpm.engine.persistence`
* `org.cadenzaflow.bpm.engine.impl.persistence.entity`
* `org.cadenzaflow.bpm.engine.impl.history.event`
* `org.cadenzaflow.bpm.engine.impl.batch`
* `org.cadenzaflow.bpm.engine.impl.cmmn.entity`
* `org.cadenzaflow.bpm.engine.impl.dmn.entity.repository`
* `org.cadenzaflow.bpm.engine.history`
...
To enable the logging for a specific database entity, please provide the namespace of the MyBatis mapper ([all mappers](https://github.com/cadenzaflow/cadenzaflow-bpm-platform/tree/master/engine/src/main/resources/org/cadenzaflow/bpm/engine/impl/mapping/entity)). Usually it is the full class name of that entity, e.g.:

* `org.cadenzaflow.bpm.engine.impl.batch.BatchEntity`
* `org.cadenzaflow.bpm.engine.impl.persistence.entity.JobEntity`
* `org.cadenzaflow.bpm.engine.impl.persistence.entity.VariableInstanceEntity`
...
* `org.cadenzaflow.bpm.engine.impl.persistence.entity.JobEntity` - logs the job execution statements
* `org.cadenzaflow.bpm.engine.jobexecutor` - further job execution logs such as job acquisition and execution operations
* `org.cadenzaflow.bpm.engine.cmd` - the start/end of commands will help to determinate in which command the job is being executed

The server log will contain logs similar to:

```plaintext
25-Nov-2019 15:45:27.613 INFO [main] o.c.c.l.BaseLogger.logInfo ENGINE-14014 Starting up the JobExecutor[org.cadenzaflow.bpm.engine.impl.jobexecutor.RuntimeContainerJobExecutor].
25-Nov-2019 15:45:27.615 INFO [Thread-6] o.c.c.l.BaseLogger.logInfo ENGINE-14018 JobExecutor[org.cadenzaflow.bpm.engine.impl.jobexecutor.RuntimeContainerJobExecutor] starting to acquire jobs
...
Find more information for Diagnosing the Job Executor in this blog post - [The Job Executor: What Is Going on in My Process Engine?](https://blog.cadenzaflow.com/post/2019/10/job-executor-what-is-going-on-in-my-process-engine/).
...
* `org.cadenzaflow.bpm.engine.persistence` - the persistence logger will log all the identity of engine entities to find the involved resources causing the deadlock
* `org.cadenzaflow.bpm.engine.cmd` - the command output will help to determinate the scope of the involved transactions which are causing the deadlock

When the issue occurs for a specific entity (e.g. `VariableInstance`), consider enabling the logger of that entity as well (`org.cadenzaflow.bpm.engine.impl.persistence.entity.VariableInstanceEntity`).
...
Some CadenzaFlow modules still use Java Util Logging directly.
...
* cadenzaflow-ejb-service
* cadenzaflow-ejb-client
* cadenzaflow-jobexecutor-ra
* cadenzaflow-jobexecutor-rar
* cadenzaflow-engine-cdi
* cadenzaflow-engine-spring
* cadenzaflow-engine-rest
