...
Job Priorities can be assigned at the process or the activity level. To achieve this the CadenzaFlow extension attribute `cadenzaflow:jobPriority` can be used.
...
When configuring job priorities at the process instance level, the `cadenzaflow:jobPriority` attribute needs to be applied to the bpmn `<process ...>` element:

```xml
<bpmn:process id="Process_1" isExecutable="true" cadenzaflow:jobPriority="8">
  ...
</bpmn:process>
```
...
```xml
<bpmn:process id="Process_1" isExecutable="true" cadenzaflow:jobPriority="${order.priority}">
  ...
</bpmn:process>
```
...
When configuring job priorities at the activity level, the `cadenzaflow:jobPriority` attribute needs to be applied
to the corresponding bpmn element:

```xml
<bpmn:serviceTask id="ServiceTask_1"
  name="Prepare Payment"
  cadenzaflow:asyncBefore="true"
  cadenzaflow:jobPriority="100" />
```
...
<bpmn:serviceTask id="ServiceTask_1"
  ...
  cadenzaflow:asyncBefore="true"
  cadenzaflow:jobPriority="${customer.status == 'VIP' ? 10 : 0}" />
```
...
However, any jobs created with a `null` value for `DUEDATE_` before disabling the optimization will not be picked up by the Job Acquisition phase, unless the jobs are explicitly updated with a due date through the **Set Due Date** {{< javadocref page="org/cadenzaflow/bpm/engine/ManagementService.html#setJobDuedate(java.lang.String,java.util.Date)" text="Java" >}}/{{< restref page="setJobDuedate" text="REST" tag="Job" >}} or **Set Retries** {{< javadocref page="org/cadenzaflow/bpm/engine/ManagementService.html#setJobRetries(int)" text="Java" >}}/{{< restref page="setJobRetries" text="REST" tag="Job" >}} APIs.
...
The CadenzaFlow engine allows you to configure this setting for the following specific elements:
...
As soon as the retry configuration is enabled, it can be applied to tasks, call activities, embedded subprocesses and transactions subprocesses. For instance, the job retry in a task can be configured in the CadenzaFlow engine in the BPMN 2.0 XML as follows:

```xml
<definitions xmlns:cadenzaflow="http://cadenzaflow.org/schema/1.0/bpmn">
  ...
  <serviceTask id="failingServiceTask" cadenzaflow:asyncBefore="true" cadenzaflow:class="org.mycompany.FailingDelegate">
    <extensionElements>
      <cadenzaflow:failedJobRetryTimeCycle>R5/PT5M</cadenzaflow:failedJobRetryTimeCycle>
    </extensionElements>
  </serviceTask>
  ...
</definitions>
...
```xml
  <cadenzaflow:failedJobRetryTimeCycle>${retryCycle}</cadenzaflow:failedJobRetryTimeCycle>
```
...
<definitions xmlns:cadenzaflow="http://cadenzaflow.org/schema/1.0/bpmn">
  ...
  <boundaryEvent id="BoundaryEvent" name="BoundaryName" attachedToRef="MyActivity">
    <extensionElements>
      <cadenzaflow:failedJobRetryTimeCycle>R3/PT5S</cadenzaflow:failedJobRetryTimeCycle>
    </extensionElements>
...
<definitions xmlns:cadenzaflow="http://cadenzaflow.org/schema/1.0/bpmn">
  ...
  <serviceTask id="failingServiceTask" cadenzaflow:asyncAfter="true" cadenzaflow:class="org.mycompany.FailingDelegate">
    <extensionElements>
      <!-- configuration for multi-instance body, e.g. after task ended -->
      <cadenzaflow:failedJobRetryTimeCycle>R5/PT5M</cadenzaflow:failedJobRetryTimeCycle>
    </extensionElements>
    <multiInstanceLoopCharacteristics isSequential="false" cadenzaflow:asyncBefore="true">
      <extensionElements>
...
```xml
<bean id="processEngineConfiguration" class="org.cadenzaflow.bpm.engine.impl.cfg.StandaloneInMemProcessEngineConfiguration">
...
**Exclusive Jobs are the default configuration**. All asynchronous continuations and timer events are thus exclusive by default. In addition, if you want a job to be non-exclusive, you can configure it as such using `cadenzaflow:exclusive="false"`. For example, the following service task would be asynchronous but non-exclusive.

```xml
<serviceTask id="service" cadenzaflow:expression="${myService.performBooking(hotel, dates)}" cadenzaflow:asyncBefore="true" cadenzaflow:exclusive="false" />
```
...
To which job acquisition a process engine is assigned can be specified in the declaration of the engine, so either in the `processes.xml` deployment descriptor of a process application or in the CadenzaFlow descriptor. The following is an example configuration that declares a new engine and assigns it to the job acquisition named `default`, which is created when the platform is bootstrapped.
...
Job acquisitions have to be declared in CadenzaFlow's deployment descriptor, see [the container-specific configuration options]({{< ref "/user-guide/runtime-container-integration/_index.md" >}}).


# Cluster Setups

When running CadenzaFlow in a cluster, there is a distinction between *homogeneous* and *heterogeneous* setups. We define a cluster as a set of network nodes that all run CadenzaFlow against the same database (at least for one engine on each node). In the *homogeneous* case, the same process applications (and thus custom classes like JavaDelegates) are deployed to all of the nodes, as depicted below.
