...
{{< enterprise >}}
  The CadenzaFlow enterprise edition provides a user interface to migrate process instances in [CadenzaFlow Cockpit]({{< ref "/webapps/cockpit/bpmn/process-instance-migration.md" >}})
{{< /enterprise >}}
...
When a user task is migrated, all properties of the task instance (i.e., `org.cadenzaflow.bpm.engine.task.Task`) are preserved apart
...
When an active [external task]({{< relref "external-tasks.md" >}}) is migrated, all properties of the external task instance (i.e., `org.cadenzaflow.bpm.engine.externaltask.ExternalTask`) are preserved
