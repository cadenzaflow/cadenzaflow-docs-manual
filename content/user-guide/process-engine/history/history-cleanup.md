...
History cleanup can be triggered manually or scheduled on a regular basis. Only [cadenzaflow-admins]({{< ref "/user-guide/process-engine/authorization-service.md#the-cadenzaflow-admin-group">}}) have permissions to execute history cleanup manually.
...
The following elements of CadenzaFlow history are cleanable:
...
* Can remove data from any CadenzaFlow version.
...
```xml
<process id="oneTaskProcess" name="The One Task Process" isExecutable="true" cadenzaflow:historyTimeToLive="5">
...
</process>
```
...
```xml
<process id="oneTaskProcess" name="The One Task Process" isExecutable="true" cadenzaflow:historyTimeToLive="P5D">
...
</process>
```
