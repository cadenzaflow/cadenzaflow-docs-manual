...
CadenzaFlow allows users to authorize access to the data it manages. This makes it possible to configure which user can access which process instances, tasks, etc...

Authorization has a performance cost and introduces some complexity. It should only be used if required.

# When is Authorization required?

Not every CadenzaFlow setup needs to enable authorization. In many scenarios, CadenzaFlow is embedded into an application and the application itself ensures that users can only access data they are authorized to access. Generally speaking, authorization is only required if untrusted parties interact with the process engine API directly. If you embed the process engine into a Java application, you usually do not need to enable authorization. The application can control how the API is accessed.

Situations in which authorization is required:

* CadenzaFlow Rest API is made accessible to users who should not have full access, even after authentication.
* CadenzaFlow Webapplication is made accessible to users who should not have full access, even after authentication.
...
* CadenzaFlow Webapplication is made accessible to users who can have full access after authentication.
...
CadenzaFlow distinguishes between two types of identities: users and groups. Authorizations can either range over all users (userId = ANY), an individual user or a group of users.
...
When using the CadenzaFlow Webapps, it is always ensured that a user is authenticated before the user can access any restricted resources.
...
The **Access** permission controls whether a user has access to a CadenzaFlow web application or not. Out of the box, it can be granted for the following applications (resource ids):
...
CadenzaFlow has no explicit concept of "administrator" beyond it being a user who has been granted all authorizations on all resources.

## The "cadenzaflow-admin" Group

When downloading the CadenzaFlow distribution, the invoice example application creates a group with id `cadenzaflow-admin` and grants all authorizations on all resources to this group.

In absense of the demo application, this task is performed by the [CadenzaFlow Admin Web Application]({{< ref "/webapps/admin/user-management.md#initial-user-setup" >}}). If the CadenzaFlow webapplication is started for the first time and no user exists in the database, it asks you to perform the "initial setup". In this process, the `cadenzaflow-admin` group is created and granted all permissions on all resources. 

{{< note title="LDAP" class="info" >}}
The group "cadenzaflow-admin" is not created when using LDAP (since LDAP is only accessed in a read-only way). Also see the below section on the administrator authorization plugin.
{{< /note >}}
...
```
# CadenzaFlow Admin Webapp

The CadenzaFlow Admin Webapplication provides an out of the box [UI for configuring Authorizations]({{< ref "/webapps/admin/authorization-management.md" >}}).
