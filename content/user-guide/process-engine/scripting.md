...
CadenzaFlow supports scripting with JSR-223 compatible script engine implementations. Currently we
...
{{< note title="" class="info" >}}
  We include **GraalVM JavaScript** in the pre-packaged CadenzaFlow distributions. 
  Consult [JavaScript Considerations](#javascript-considerations) for further information.
  
  We include **Groovy** in the pre-packaged CadenzaFlow distributions.
{{< /note >}}
...
<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"
                   targetNamespace="http://cadenzaflow.org/example">
...
Besides Java code and expression language, CadenzaFlow also supports the execution of a script
...
To use a script as an execution listener, a `cadenzaflow:script` element has to be added as a child
element of the `cadenzaflow:executionListener` element. During script evaluation, the variable `execution` is
...
<process id="process" isExecutable="true">
  <extensionElements>
    <cadenzaflow:executionListener event="start">
  ...
  <startEvent id="start">
    <extensionElements>
      <cadenzaflow:executionListener event="end">
        <cadenzaflow:script scriptFormat="groovy">
  ...
  <sequenceFlow id="flow1" startRef="start" targetRef="task">
    <extensionElements>
      <cadenzaflow:executionListener>
        <cadenzaflow:script scriptFormat="groovy" resource="org/cadenzaflow/bpm/transition.groovy" />
      </cadenzaflow:executionListener>
    </extensionElements>
  </sequenceFlow>
...
To use a script as a task listener, a `cadenzaflow:script` element has to be added as a child element of
the `cadenzaflow:taskListener` element. Inside the script, the variable `task` is available, which corresponds to
...
<userTask id="userTask">
  <extensionElements>
    <cadenzaflow:taskListener event="create">
      <cadenzaflow:script scriptFormat="groovy">println task.eventName</cadenzaflow:script>
    </cadenzaflow:taskListener>
    <cadenzaflow:taskListener event="assignment">
      <cadenzaflow:script scriptFormat="groovy" resource="org/cadenzaflow/bpm/assignemnt.groovy" />
    </cadenzaflow:taskListener>
  </extensionElements>
</userTask>
```
...
As an alternative to expression language, CadenzaFlow allows you to use scripts as
...
<sequenceFlow>
  <conditionExpression xsi:type="tFormalExpression" language="groovy"
      cadenzaflow:resource="org/cadenzaflow/bpm/condition.groovy" />
</sequenceFlow>
```

# Use Scripts as inputOutput Parameters

With the CadenzaFlow `inputOutput` extension element you can map an `inputParameter` or `outputParameter`
...
<?xml version="1.0" encoding="UTF-8"?>
<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"
                   xmlns:cadenzaflow="http://activiti.org/bpmn"
                   targetNamespace="http://cadenzaflow.org/example">
  <process id="process" isExecutable="true">
    <startEvent id="start"/>
    <sequenceFlow id="sequenceFlow1" sourceRef="start" targetRef="task"/>
    <serviceTask id="task" cadenzaflow:class="org.cadenzaflow.bpm.example.SumDelegate">
      <extensionElements>
        <cadenzaflow:inputOutput>
          <cadenzaflow:inputParameter name="x">
             <cadenzaflow:script scriptFormat="groovy">
              <![CDATA[
...
<cadenzaflow:inputOutput>
  <cadenzaflow:inputParameter name="x">
     <cadenzaflow:script scriptFormat="groovy" resource="org/cadenzaflow/bpm/example/sum.groovy"/>
  </cadenzaflow:inputParameter>
</cadenzaflow:inputOutput>
```
...
Consult your application server's documentation to learn how to pass on JVM arguments. CadenzaFlow Run supports setting 
...
You can inherit from the `org.cadenzaflow.bpm.engine.impl.scripting.engine.DefaultScriptEngineResolver` for starters in case configuring an existing 
...
1. `execution`, which is always available if the script is executed in an execution scope (e.g., in a script task) ({{< javadocref page="org/cadenzaflow/bpm/engine/delegate/DelegateExecution.html" text="DelegateExecution" >}}).
1. `task`, which is available if the script is executed in a task scope (e.g., a task listener) ({{< javadocref page="org/cadenzaflow/bpm/engine/delegate/DelegateTask.html" text="DelegateTask" >}}).
1. `connector`, which is available if the script is executed in a connector variable scope (e.g., outputParameter of a cadenzaflow:connector) ({{< javadocref page="org/cadenzaflow/connect/plugin/impl/ConnectorVariableScope.html" text="ConnectorVariableScope" >}}).
...
CadenzaFlow's Java API provides access to CadenzaFlow's process engine services; these services can be accessed using Scripts:

{{< javadocref page="org/cadenzaflow/bpm/engine/ProcessEngineServices.html" text="Process Engine Services" >}} \

{{< javadocref page="org/cadenzaflow/bpm/engine/package-summary.html" text="Public Java API of CadenzaFlow Engine" >}}
...
the XML file. Nonetheless, CadenzaFlow provides additional ways to specify the script source.
...
<cadenzaflow:executionListener>
  <cadenzaflow:script scriptFormat="groovy">${sourceCode}</cadenzaflow:script>
</cadenzaflow:executionListener>
...
You can also specify the attribute `cadenzaflow:resource` on the `scriptTask` and `conditionExpression`
element, respectively the `resource` attribute on the `cadenzaflow:script` element. This extension
...
```xml
<!-- on a script task -->
<scriptTask scriptFormat="groovy" cadenzaflow:resource="org/cadenzaflow/bpm/task.groovy"/>
<scriptTask scriptFormat="groovy" cadenzaflow:resource="classpath://org/cadenzaflow/bpm/task.groovy"/>
<scriptTask scriptFormat="groovy" cadenzaflow:resource="deployment://org/cadenzaflow/bpm/task.groovy"/>

<!-- in an execution listener -->
<cadenzaflow:executionListener>
  <cadenzaflow:script scriptFormat="groovy" resource="deployment://org/cadenzaflow/bpm/listener.groovy"/>
</cadenzaflow:executionListener>
...
<cadenzaflow:inputParameter name="x">
  <cadenzaflow:script scriptFormat="groovy" resource="org/cadenzaflow/bpm/mapX.groovy" />
</cadenzaflow:inputParameter>
```
...
```xml
<scriptTask scriptFormat="groovy" cadenzaflow:resource="${scriptPath}"/>
```

For more information, see the
[cadenzaflow:resource]({{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#resource" >}})
...
We include **GraalVM JavaScript** in the pre-packaged CadenzaFlow distributions as a replacement regardless of the JRE version.
