...
CadenzaFlow supports the Unified Expression Language (EL), specified by the [Jakarta Expression
...
Within CadenzaFlow, EL can be used in many circumstances to evaluate small script-like
...
Besides Java code, CadenzaFlow also supports the evaluation of expressions as delegation code. For
general information about delegation code, see the corresponding
[section]({{< ref "/user-guide/process-engine/delegation-code.md" >}}).

Two types of expressions are currently supported: `cadenzaflow:expression` and
`cadenzaflow:delegateExpression`.

With `cadenzaflow:expression` it is possible to evaluate a value expression or to invoke
...
```xml
  <process id="process">
    <extensionElements>
      ...
      <cadenzaflow:executionListener event="start" expression="${execution.setVariable('test', 'foo')}" />
    </extensionElements>
    ...
    <userTask id="userTask">
      <extensionElements>
        ...
        <cadenzaflow:taskListener event="complete" expression="${myBean.taskDone(task)}" />
      </extensionElements>
    </userTask>
    ...
    <serviceTask id="serviceTask"
        cadenzaflow:expression="${myBean.ready}" cadenzaflow:resultVariable="myVar" />
...
The attribute `cadenzaflow:delegateExpression` is used for expressions which evaluate to a delegate
...
```xml
  ...
  <serviceTask id="task1" cadenzaflow:delegateExpression="${myBean}" />

  <!-- service task which calls a method which returns delegate object -->
  <serviceTask id="task2" cadenzaflow:delegateExpression="${myBean.createDelegate()}" />
```
...
[cadenzaflow:errorEventDefinition]({{< ref "/reference/bpmn20/custom-extensions/extension-elements.md#erroreventdefinition" >}})
...
In the scope of an External Task, expressions have access to the {{< javadocref page="org/cadenzaflow/bpm/engine/externaltask/ExternalTask.html" text="ExternalTaskEntity" >}} object via the key `externalTask` which provides getter methods for `errorMessage`, `errorDetails`, `workerId`, `retries` and more.
...
```xml
<bpmn:serviceTask id="myExternalTaskId" name="myExternalTask" cadenzaflow:type="external" cadenzaflow:topic="myTopic">
  <bpmn:extensionElements>
    <cadenzaflow:errorEventDefinition id="myErrorEventDefinition" errorRef="myError" expression="${externalTask.getWorkerId() == 'myWorkerId'}" />
  </bpmn:extensionElements>
</bpmn:serviceTask>
```
...
<bpmn:serviceTask id="myExternalTaskId" name="myExternalTask" cadenzaflow:type="external" cadenzaflow:topic="myTopic">
  <bpmn:extensionElements>
    <cadenzaflow:errorEventDefinition id="myErrorEventDefinition" errorRef="myError" expression="${externalTask.getErrorDetails().contains('myErrorMessage')}" />
  </bpmn:extensionElements>
</bpmn:serviceTask>
```
...
<table class="table">
  ...
  <tbody>
    <tr>
      ...
      <td><code>{{< javadocref page="org/cadenzaflow/bpm/engine/delegate/DelegateExecution.html" text="DelegateExecution" >}}</code></td>
      <td>
    ...
    <tr>
      ...
      <td><code>{{< javadocref page="org/cadenzaflow/bpm/engine/delegate/DelegateTask.html" text="DelegateTask" >}}</code></td>
      <td>Available in a task context like a task listener.</td>
    </tr>
    <tr>
      <td><code>externalTask</code></td>
      <td><code>{{< javadocref page="org/cadenzaflow/bpm/engine/externaltask/ExternalTask.html" text="ExternalTask" >}}</code></td>
      <td>Available during an external task context activity (e.g. in <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-elements.md#erroreventdefinition" >}}">cadenzaflow:errorEventDefinition</a> expressions).</td>
    </tr>
    <tr>
      <td><code>caseExecution</code></td>
      <td><code>{{< javadocref page="org/cadenzaflow/bpm/engine/delegate/DelegateCaseExecution.html" text="DelegateCaseExecution" >}}</code></td>
...
```xml
  <serviceTask id="task2" cadenzaflow:expression="${myBean.myMethod(execution)}" />
```
...
```xml
<userTask id="theTask" name="Important task" cadenzaflow:dueDate="${dateTime().plusDays(3).toDate()}"/>
```


## Built-In CadenzaFlow Spin Functions

If the CadenzaFlow Spin process engine plugin is activated, the Spin functions `S`,
...
```xml
  <serviceTask id="task" cadenzaflow:expression="${XML(xml).attr('test').value()}" resultVariable="test" />
```
