...
Please note that the WildFly and JBoss application servers do not support FEEL outside of DMN execution in CadenzaFlow.
{{< /note >}}
...
Within CadenzaFlow, FEEL can be used in many circumstances to evaluate small script-like expressions. The following table provides an overview of the BPMN elements which support usage of FEEL.
...
## Custom Functions
CadenzaFlow provides a wrapper for the FEEL Scala Engine to implement Custom Functions, which can be 
...
To learn how CadenzaFlow Spin can be used together with the Scala FEEL Engine visit the <a href="{{< ref "/user-guide/dmn-engine/feel/type-handling.md#spin-types" >}}">FEEL Engine Spin Integration</a> page.
...
<a href="{{< ref "/user-guide/data-formats/configuring-spin-integration#cadenzaflow-engine-plugin-spin" >}}">cadenzaflow-engine-plugin-spin</a>
...
```xml
<cadenzaflow:inputParameter name="context">
  <cadenzaflow:script scriptFormat="feel">
    {
      invoice: {
        id: "INV-2024-001",
...
To access this `context` as a `Map<String, Object>` in a Java delegate, the `cadenzaflow-engine-plugin-spin` plugin must be available so that the nested structure is correctly mapped.
