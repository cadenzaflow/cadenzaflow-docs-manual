...
*Multi-Tenancy* regards the case in which a single CadenzaFlow installation should serve more than one tenant. For each tenant, certain guarantees of isolation should be made. For example, one tenant's process instances should not interfere with those of another tenant.
...
Find [examples on GitHub](https://github.com/cadenzaflow/cadenzaflow-bpm-examples) that show how to use tenant-identifiers with

* [Embedded Process Engine](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/multi-tenancy/tenant-identifier-embedded)
* [Shared Process Engine](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/multi-tenancy/tenant-identifier-shared)
{{< /note >}}
...
When a deployment is created using the Repository Service, the tenant identifier can be set on the {{< javadocref page="org/cadenzaflow/bpm/engine/repository/DeploymentBuilder.html" text="DeploymentBuilder" >}}.
...
```xml
<bean id="processEngineConfiguration" class="org.cadenzaflow.bpm.engine.spring.SpringProcessEngineConfiguration">
  <property name="deploymentResources">
    <array>
      <value>classpath*:/org/cadenzaflow/bpm/engine/spring/test/autodeployment/autodeploy.*.cmmn</value>
      <value>classpath*:/org/cadenzaflow/bpm/engine/spring/test/autodeployment/autodeploy.*.bpmn20.xml</value>
    </array>
  </property>
...
To create an instance by key of a process definition which is deployed for multiple tenants, the tenant identifier has to be passed to the {{< javadocref page="org/cadenzaflow/bpm/engine/runtime/ProcessInstantiationBuilder.html" text="ProcessInstantiationBuilder" >}}. 
...
The [Signal API]({{< ref "/reference/bpmn20/events/signal-events.md#signal-api" >}}) can be used to deliver a signal to one or all tenants. Pass the tenant identifier to the {{< javadocref page="org/cadenzaflow/bpm/engine/runtime/SignalEventReceivedBuilder.html" text="SignalEventReceivedBuilder" >}} to deliver the signal to a specific tenant. If no identifier is passed then the signal is delivered to all tenants.
...
To create an instance by key of a case definition which is deployed for multiple tenants, the tenant identifier has to be passed to the {{< javadocref page="org/cadenzaflow/bpm/engine/runtime/CaseInstanceBuilder.html" text="CaseInstanceBuilder" >}}.
...
To evaluate a decision table by key which is deployed for multiple tenants, the tenant identifier has to be passed to the {{< javadocref page="org/cadenzaflow/bpm/engine/dmn/DecisionEvaluationBuilder.html" text="DecisionEvaluationBuilder" >}}.
...
When integrating CadenzaFlow into an application, it can be cumbersome to pass the tenant Id to each cadenzaflow API call. Since such an application usually also has a concept of an "authenticated user", it is possible to set the list of tenant ids when setting the authentication:
...
### CadenzaFlow Rest API and Web Applications

The CadenzaFlow [Rest API]({{< ref "/reference/rest/_index.md" >}}) and the web applications Cockpit and Tasklist support the transparent access restrictions. When a user logs in then he only sees and can only access the data (e.g., process definitions) that belongs to one of his tenants.
...
By the term *usage pattern* we mean that it is not a feature of CadenzaFlow per se but rather a specific way to use it to achieve the desired behavior.

{{< note title="Example" class="info" >}}
You can find an [example](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/multi-tenancy/tenant-identifier-shared-definitions) on [GitHub](https://github.com/cadenzaflow/cadenzaflow-bpm-examples) that shows how to use shared definitions.
{{< /note >}}
...
Shared resources  do not have a tenant id which means that no tenant id is propagated automatically. To have the tenant id of the user who starts the process instances assigned  to the process instance, an implementation of the {{< javadocref page="org/cadenzaflow/bpm/engine/impl/cfg/multitenancy/TenantIdProvider.html" text="TenantIdProvider" >}} SPI needs to be provided.
...
To use the `TenantIdProvider`, it must be set in the Process Engine Configuration, for example using the `cadenzaflow.cfg.xml`:

```xml
<beans>
  <bean id="processEngineConfiguration" class="org.cadenzaflow.bpm.engine.impl.cfg.StandaloneProcessEngineConfiguration">
    <!-- ... -->
    
    <property name="tenantIdProvider" ref="tenantIdProvider" />
  </bean>
  
  <bean id="tenantIdProvider" class="org.cadenzaflow.bpm.CustomTenantIdProvider">
</beans>
```
...
To realize this, the call activity or business rule task needs to select the correct definition to invoke based on the tenant id of the current process instance. The [Shared Resources Example](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/multi-tenancy/tenant-identifier-shared-definitions) shows how to achieve this.

See also:

* [Shared Resources Example](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/multi-tenancy/tenant-identifier-shared-definitions)
...
{{< note title="Tutorial" class="info" >}}
  You can see the [example](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/multi-tenancy/schema-isolation) how to implement multi-tenancy with data isolation by schemas.
{{< /note >}}
...
<bpm-platform xmlns="http://www.cadenzaflow.org/schema/1.0/BpmPlatform"
  ...
  <process-engine name="tenant1">
    ...
    <configuration>org.cadenzaflow.bpm.engine.impl.cfg.StandaloneProcessEngineConfiguration</configuration>
...
To access a specific tenant's process engine at runtime, it has to be identified by its name. The CadenzaFlow engine offers access to named engines in various programming models:
...
The CadenzaFlow web applications Cockpit, Tasklist and Admin offer tenant-specific views out of the box by [switching between different process engines]({{< ref "/webapps/cockpit/dashboard.md#multi-engine" >}}).
