...
The CadenzaFlow engine uses the default time zone of the JVM when operating with dates:
...
Database time zone and database sessions time zone are out of scope of the CadenzaFlow engine and must be configured explicitly.

However, Timestamp columns in the CadenzaFlow engine are using the `TIMESTAMP [WITHOUT TIME ZONE]` data type (the name differs in different database servers).
For this reason, it is not recommended to change the time zone on the database side once set, since it may lead to an incorrect operation of the CadenzaFlow engine.
...
It is possible to use the CadenzaFlow Web Applications in different timezones. All dates are translated to/from the local timezone when working with the UI.
