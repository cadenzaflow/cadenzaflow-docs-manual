...
<cadenzaflow:executionListener event="end">
        <cadenzaflow:script scriptFormat="groovy"><![CDATA[execution.setVariable("aVariable", "aValue","aSubProcess");]]></cadenzaflow:script>
...
configured charset, this length restriction can result in different quantities of real characters. Variable value length is not validated inside the CadenzaFlow engine, but
 ...
 it may be implemented separately and must happen before the CadenzaFlow API to set the variables is called.
{{< /note >}}

Process variables can be stored in formats like JSON and XML provided by the [CadenzaFlow Spin plugin]({{< ref "/user-guide/data-formats/_index.md" >}}). Spin provides serializers for the variables of type `object` such that Java variables can be persisted in these formats to the database. Furthermore, it is possible to store JSON and XML documents directly as a Spin object by the value types `xml` and `json`. Opposed to plain `string` variables, Spin objects provide a fluent API to perform common operations on such documents like reading and writing properties.
...
{{< note title="Serializing Objects to XML and JSON" class="info" >}}
  The [CadenzaFlow Spin plugin]({{< ref "/user-guide/data-formats/_index.md" >}}) provides serializers that are capable of serializing object values to XML and JSON. They can be used when it is desired that the serialized objects values can be interpreted by humans or when the serialized value should be meaningful without having the corresponding Java class. When using a pre-built CadenzaFlow distribution, CadenzaFlow Spin is already preconfigured and you can try these formats without further configuration.
{{< /note >}}
...
In order to easily construct typed values, CadenzaFlow offers the class `org.cadenzaflow.bpm.engine.variable.Variables`. This class contains static methods that allow creation of single typed values as well as creation of a map of typed values in a fluent way.
...
To improve the reusability of source code and business logic, CadenzaFlow offers input/output
mapping of process variables. This can be used for tasks, events and subprocesses.

In order to use the variable mapping, the CadenzaFlow extension element [inputOutput][] has to be added
...
<cadenzaflow:inputOutput>
  <cadenzaflow:inputParameter name="x">foo</cadenzaflow:inputParameter>
  <cadenzaflow:inputParameter name="willBeNull"/>
  <cadenzaflow:outputParameter name="y">${x}</cadenzaflow:outputParameter>
  <cadenzaflow:outputParameter name="z">${willBeNull == null}</cadenzaflow:outputParameter>
</cadenzaflow:inputOutput>
```
...
<cadenzaflow:inputOutput>
  <cadenzaflow:inputParameter name="x">
    <cadenzaflow:list>
      <cadenzaflow:value>a</cadenzaflow:value>
      <cadenzaflow:value>${1 + 1}</cadenzaflow:value>
      <cadenzaflow:list>
        <cadenzaflow:value>1</cadenzaflow:value>
        <cadenzaflow:value>2</cadenzaflow:value>
        <cadenzaflow:value>3</cadenzaflow:value>
      </cadenzaflow:list>
    </cadenzaflow:list>
  </cadenzaflow:inputParameter>
  <cadenzaflow:outputParameter name="y">
    <cadenzaflow:map>
      <cadenzaflow:entry key="foo">bar</cadenzaflow:entry>
      <cadenzaflow:entry key="map">
        <cadenzaflow:map>
          <cadenzaflow:entry key="hello">world</cadenzaflow:entry>
          <cadenzaflow:entry key="cadenzaflow">bpm</cadenzaflow:entry>
        </cadenzaflow:map>
      </cadenzaflow:entry>
    </cadenzaflow:map>
...
In more detail, let us assume such a calculation is implemented by a Java Delegate class `org.cadenzaflow.bpm.example.ComplexCalculation`.
...
```xml
<serviceTask cadenzaflow:class="org.cadenzaflow.bpm.example.ComplexCalculation">
  <extensionElements>
    <cadenzaflow:inputOutput>
      <cadenzaflow:inputParameter name="userId">${id}</cadenzaflow:inputParameter>
      <cadenzaflow:inputParameter name="costSum">${sum}</cadenzaflow:inputParameter>
      <cadenzaflow:outputParameter name="forecast">${realisticForecast}</cadenzaflow:outputParameter>
    </cadenzaflow:inputOutput>
  </extensionElements>
</serviceTask>
```
...
<serviceTask cadenzaflow:class="org.cadenzaflow.bpm.example.ComplexCalculation">
  <extensionElements>
    <cadenzaflow:inputOutput>
      <cadenzaflow:inputParameter name="userId">${id}</cadenzaflow:inputParameter>
      <cadenzaflow:inputParameter name="costSum">
        ${mapA[costs] + mapB[costs] + mapC[costs]}
      </cadenzaflow:inputParameter>
      <cadenzaflow:outputParameter name="avgForecast">
...
[inputOutput]: {{< ref "/reference/bpmn20/custom-extensions/extension-elements.md#cadenzaflow-inputoutput" >}}
...
[list]: {{< ref "/reference/bpmn20/custom-extensions/extension-elements.md#cadenzaflow-list" >}}
[map]: {{< ref "/reference/bpmn20/custom-extensions/extension-elements.md#cadenzaflow-map" >}}
