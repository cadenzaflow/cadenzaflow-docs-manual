...
To implement a class that can be called during process execution, this class needs to implement the `org.cadenzaflow.bpm.engine.delegate.JavaDelegate`
...
the `org.cadenzaflow.bpm.engine.delegate.JavaDelegate`
...
manipulated through the {{< javadocref page="org/cadenzaflow/bpm/engine/delegate/DelegateExecution.html" text="DelegateExecution" >}} interface (click on the link for a detailed Javadoc of its operations).
...
`cadenzaflow:class`  ) are **NOT instantiated during deployment**.
...
Instead of writing a Java Delegate, it is also possible to provide a class that implements the `org.cadenzaflow.bpm.engine.impl.pvm.delegate.ActivityBehavior`
...
setter/private field on the injection target should always be `org.cadenzaflow.bpm.engine.delegate.Expression`**.
...
```xml
  <serviceTask id="javaService"
               ...
               cadenzaflow:class="org.cadenzaflow.bpm.examples.bpmn.servicetask.ToUpperCaseFieldInjected">
    <extensionElements>
        <cadenzaflow:field name="text" stringValue="Hello World" />
    </extensionElements>
  </serviceTask>
```

The class `ToUpperCaseFieldInjected` has a field
`text` which is of type `org.cadenzaflow.bpm.engine.delegate.Expression`.
When calling `text.getValue(execution)`, the configured string value
`Hello World` will be returned.

Alternatively, for longs texts (e.g., an inline e-mail) the `cadenzaflow:string` sub element can be
used:
...
`org.cadenzaflow.bpm.engine.delegate.Expression`
which can be evaluated/invoked using the `DelegateExecution`
passed in the `execute` method.

```xml
  <serviceTask id="javaService" name="Java service invocation"
               cadenzaflow:class="org.cadenzaflow.bpm.examples.bpmn.servicetask.ReverseStringsFieldInjected">

    <extensionElements>
      <cadenzaflow:field name="text1">
        <cadenzaflow:expression>${genderBean.getGenderString(gender)}</cadenzaflow:expression>
      </cadenzaflow:field>
      <cadenzaflow:field name="text2">
         <cadenzaflow:expression>Hello ${gender == 'male' ? 'Mr.' : 'Mrs.'} ${name}</cadenzaflow:expression>
      </cadenzaflow:field>
    </extensionElements>
  </serviceTask>
```
...
```xml
  <cadenzaflow:field name="text1" expression="${genderBean.getGenderString(gender)}" />
  <cadenzaflow:field name="text2" expression="Hello ${gender == 'male' ? 'Mr.' : 'Mrs.'} ${name}" />
```
...
To implement a class that delegates the input and output variable mapping for a call activity, this class needs to implement the `org.cadenzaflow.bpm.engine.delegate.DelegateVariableMapping`
...
```xml
  <process id="executionListenersProcess">
    <extensionElements>
      <cadenzaflow:executionListener
          event="start"
          class="org.cadenzaflow.bpm.examples.bpmn.executionlistener.ExampleExecutionListenerOne" />
    </extensionElements>
    ...
    <sequenceFlow sourceRef="firstTask" targetRef="secondTask">
      <extensionElements>
        <cadenzaflow:executionListener>
          <cadenzaflow:script scriptFormat="groovy">
    ...
    <userTask id="secondTask">
      <extensionElements>
        <cadenzaflow:executionListener expression="${myPojo.myMethod(execution.eventName)}" event="end" />
      </extensionElements>
    </userTask>
...
The first execution listener is notified when the process starts. The listener is an external Java-class (like ExampleExecutionListenerOne) and should implement the `org.cadenzaflow.bpm.engine.delegate.ExecutionListener` interface. When the event occurs (in this case end event) the method `notify(DelegateExecution execution)` is called.
...
It is also possible to use a delegation class that implements the `org.cadenzaflow.bpm.engine.delegate.JavaDelegate` interface. These delegation classes can then be reused in other constructs, such as a delegation for a service task.
...
[cadenzaflow:script][cadenzaflow-script] child element which defines a script which
...
```xml
  <cadenzaflow:executionListener event="start" delegateExpression="${myExecutionListenerBean}" />
```


# Task Listener

A task listener is used to execute custom Java logic or an expression upon the occurrence of a certain task-related event. It can only be added in the process definition as a child element of a user task. Note that this also must happen as a child of the BPMN 2.0 extensionElements and in the CadenzaFlow namespace, since a task listener is a construct specifically for the CadenzaFlow engine.

```xml
  <userTask id="myTask" name="My Task" >
    <extensionElements>
      <cadenzaflow:taskListener event="create" class="org.cadenzaflow.bpm.MyTaskCreateListener" />
    </extensionElements>
  </userTask>
```
...
* **class**: the delegation class that must be called. This class must implement the `org.cadenzaflow.bpm.engine.impl.pvm.delegate.TaskListener` interface.
...
* **expression**: (cannot be used together with the class attribute): specifies an expression that will be executed when the event happens. It is possible to pass the DelegateTask object and the name of the event (using task.eventName) to the called object as parameters.
    ...
    ```xml
    <cadenzaflow:taskListener event="create" expression="${myObject.callMethod(task, task.eventName)}" />
    ```
...
* **delegateExpression**: allows to specify an expression that resolves to an object implementing the TaskListener interface, similar to a service task.
    ...
    ```xml
    <cadenzaflow:taskListener event="create" delegateExpression="${myTaskListenerBean}" />
    ```
...
[cadenzaflow:script][cadenzaflow-script] child element can be used to specify a script as task listener.
An external script resource can also be declared with the resource attribute of the
`cadenzaflow:script` element (see the documentation about [script sources][script-sources] of script
tasks).

```xml
  <userTask id="task">
    <extensionElements>
      <cadenzaflow:taskListener event="create">
        <cadenzaflow:script scriptFormat="groovy">
          println task.eventName
        </cadenzaflow:script>
      </cadenzaflow:taskListener>
    </extensionElements>
  </userTask>
```
...
```xml
  <userTask id="task">
    <extensionElements>
      <cadenzaflow:taskListener event="timeout" delegateExpression="${myTaskListenerBean}" id="friendly-reminder" >
...
```xml
  <process id="executionListenersProcess">
    <extensionElements>
      <cadenzaflow:executionListener class="org.cadenzaflow.bpm.examples.bpmn.executionListener.ExampleFieldInjectedExecutionListener" event="start">
        <cadenzaflow:field name="fixedValue" stringValue="Yes, I am " />
        <cadenzaflow:field name="dynamicValue" expression="${myVar}" />
      </cadenzaflow:executionListener>
    </extensionElements>
...
```java
  @Deployment(resources = {
    "org/cadenzaflow/bpm/examples/bpmn/executionListener/ExecutionListenersFieldInjectionProcess.bpmn20.xml"
  })
...
You can throw a `{{< javadocref page="org/cadenzaflow/bpm/engine/ProcessEngineException.html" text="ProcessEngineException" >}}`
...
the code via `ProcessEngineException#getCode` when calling CadenzaFlow Java API or by evaluating the 
...
[cadenzaflow-script]: {{< ref "/reference/bpmn20/custom-extensions/extension-elements.md#cadenzaflow-script" >}}
