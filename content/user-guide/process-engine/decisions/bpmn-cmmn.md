...
<definitions id="taskAssigneeExample"
  ...
  xmlns:cadenzaflow="http://cadenzaflow.org/schema/1.0/bpmn"
...
{{< javadocref page="org/cadenzaflow/bpm/dmn/engine/DmnDecisionResult"
text="Java Docs" >}}.
...
mapping, the `cadenzaflow:resultVariable` attribute is used.
...
```xml
<businessRuleTask id="businessRuleTask" cadenzaflow:decisionRef="myDecision">
  <extensionElements>
    <cadenzaflow:inputOutput>
      ...
      <cadenzaflow:outputParameter name="reason">
...
<decisionTask id="decisionTask" decisionRef="myDecision">
  <extensionElements>
    <cadenzaflow:caseExecutionListener event="complete"
      class="org.cadenzaflow.bpm.example.MyDecisionResultListener" />
  </extensionElements>
</decisionTask>
```
...
the underlying implementation can change in a new version of CadenzaFlow.
...
If the DMN engine is invoked by CadenzaFlow, it uses the same
JUEL configuration as the CadenzaFlow engine. Therefore, it is also
...
page="?org/cadenzaflow/bpm/engine/impl/javax/el/FunctionMapper.html"
