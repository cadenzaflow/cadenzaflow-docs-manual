...
For working with XML, the Spin functions `org.cadenzaflow.spin.Spin.S` and `org.cadenzaflow.spin.Spin.XML` can be used as entry points. The latter offers strongly-typed access to Spin's XML API and is preferable when writing Java code. In scripting environments, only the `S` function is available. The returned Spin wrapper offers methods for manipulating and writing XML as well as mapping XML to Java. Furthermore, the entry functions can be provided with Java objects that get implicitly converted to Spin's intermediary XML format.

The following provides examples on how CadenzaFlow Spin can be used in the process engine to work with XML data. For illustration purposes, let us assume that a String process variable `customer` containing XML exists. It has the following content:

```xml
<?xml version="1.0" encoding="UTF-8"?>
<customer xmlns="http://cadenzaflow.org/example" name="Jonny">
  <address>
...
Further documentation about the usage of Spin can be found in the [CadenzaFlow Spin Dataformat Reference]({{< ref "/reference/spin/_index.md" >}}).
...
public class MyDelegate implements JavaDelegate {
  ...
  public void execute(DelegateExecution execution) throws Exception {
    String xml = "<customer xmlns=\"http:\\/\\/cadenzaflow.org/example\" name=\"Jonny\">"
          + "<address>"
            + "<street>12 High Street</street>"
            + "<postCode>1234</postCode>"
          + "</address>"
...
public class AddDataDelegate implements JavaDelegate {
  ...
  public void execute(DelegateExecution execution) throws Exception {
    ...
    //<?xml version="1.0" encoding="UTF-8"?><customer xmlns="http:\/\/cadenzaflow.org/example" name="Jonny"><address><street>12 High Street</street><postCode>1234</postCode></address><creditLimit xmlns="">1000.00</creditLimit></customer>
  }
}
```
...
```java
@XmlRootElement(namespace = "http://cadenzaflow.org/example")
public class Customer {
  ...
  @XmlElement(namespace = "http://cadenzaflow.org/example")
