...
A working example can be found in the [examples repository](https://github.com/cadenzaflow/cadenzaflow-bpm-examples/tree/master/usertask/task-form-external-jsf).
...
This is how the forms are referenced in the BPMN 2.0 XML with the `cadenzaflow:formKey` attribute:

```xml
<startEvent id="start" name="invoice received"
    cadenzaflow:formKey="app:sample-start-form.jsf"/>

<userTask id="categorize-invoice" name="Categorize Invoice"
    cadenzaflow:formKey="app:sample-task-form-1.jsf" />

<userTask id="file-invoice" name="File Invoice"
    cadenzaflow:formKey="app:sample-task-form-2.jsf" />

<userTask id="acknowledge-categorization" name="Acknowledge Categorization"
    cadenzaflow:formKey="app:acknowledge-form.jsf" />
```
  ...
  <h:body>
    ...
    <h:form id="someForm">
      ...
      <h:commandButton id="complete" value="Task Completed"
          action="#{cadenzaflowTaskForm.completeTask()}" />
    </h:form>
  </h:body>
</f:view>
</html>
...
Submit the form by calling the `cadenzaflowTaskForm` bean again, which:
...
```xml
<h:commandButton id="complete" value="task completed" action="#{cadenzaflowTaskForm.completeTask()}" />
```
...
In the forms you can access your own CDI beans as usual and also access the CadenzaFlow CDI beans. This makes it easy to access process variables, e.g., via the `processVariables` CDI bean:
...
<f:view>
  ...
  <h:head>
    <title>Start Process: #{cadenzaflowTaskForm.processDefinition.name}</title>
  </h:head>
  <h:body>
    <h1>#{cadenzaflowTaskForm.processDefinition.name}</h1>
    <p>Start a new process instance in version: #{cadenzaflowTaskForm.processDefinition.version}</p>
    <h:form id="someForm">
      <p>
        ...
        <tr>
          ...
          <td>
            <h:commandButton id="start" value="Start Process Instance"
                action="#{cadenzaflowTaskForm.completeProcessInstanceForm()}" />
          </td>
        </tr>
      </table>
    </h:form>
...
```xml
<f:metadata>
  <f:event type="preRenderView" listener="#{cadenzaflowTaskForm.startProcessInstanceByIdForm()}" />
</f:metadata>
```
...
```xml
<h:commandButton id="start" value="Start Process Instance" action="#{cadenzaflowTaskForm.completeProcessInstanceForm()}" />
```
