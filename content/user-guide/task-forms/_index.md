...
Forms are referenced using Form Keys or Form References and can either be embedded in CadenzaFlow Tasklist or handled by a custom application. Depending on your use-case, different Form Types can be used:

1. [Embedded Task Forms](#embedded-task-forms) allow you to embed custom HTML and JavaScript forms into Tasklist.
2. [CadenzaFlow Forms](#cadenzaflow-forms) offer visual editing of forms in the CadenzaFlow Modeler and can be used for less complex forms. CadenzaFlow Forms are the only form type that can be referenced either by Form Key or by Form Reference.
...
<table class="table table-striped">
  ...
  <tr>
    <td>FORM-TYPE</td>
    <td>Can be <code>embedded</code> or <code>cadenzaflow-forms</code> depending on the form type. If no type is set, the form will be shown as an <a href="#external-task-forms">External Task Form</a>.</td>
  </tr>
  <tr>
    <td>LOCATION</td>
    <td>Can be either <code>deployment</code> or <code>app</code>:
    <ul>
      <li><em>deployment</em>: The file is part of your deployment (e.g., <a href="{{< ref "/reference/deployment-descriptors/tags/process-archive.md" >}}">by adding it to your process archive</a> or <a href="https://docs.cadenzaflow.org/get-started/quick-start/deploy/#use-the-cadenzaflow-modeler-to-deploy-the-process">by deploying from the CadenzaFlow Modeler</a>), which means that it is stored in the CadenzaFlow database. It can then be loaded from there. Note that this allows versioning of your form alongside the process model.</li>
...
To configure the form in your process, open the process with the [CadenzaFlow Modeler](http://cadenzaflow.org/bpmn/tool/) and select the desired [UserTask][user-tasks] or [StartEvent][start-event]. Open the properties panel and enter the Form Key. The relevant XML tag looks like this:

```xml
<userTask id="theTask" cadenzaflow:formKey="cadenzaflow-forms:deployment:forms/userTask.form"
          cadenzaflow:candidateUsers="John, Mary"
          name="my Task">
```
...
CadenzaFlow Forms are created as separate files using the CadenzaFlow Modeler and can be deployed together with the process models. The form schema is stored in `.form` files.  You can find out how to build CadenzaFlow Forms in the [CadenzaFlow Modeler documentation]({{< ref "/modeler/forms.md" >}}) or refer to the [CadenzaFlow Forms Reference](https://docs.cadenzaflow.io/docs/guides/utilizing-forms/) to explore all configuration options for form elements.
...
With Form References, CadenzaFlow Forms provide a flexible way of linking an element in a BPMN diagram to a form. To link a BPMN element ([StartEvent][start-event] or [UserTask][user-tasks]) to a CadenzaFlow Form, you have to specify the Id of the CadenzaFlow Form as the `cadenzaflow:formRef` attribute. Additionally, the `cadenzaflow:formRefBinding` attribute specifies which version of the CadenzaFlow Form to reference.

Valid values are:

* `deployment`, which references the CadenzaFlow Form with the given key that was deployed with the same deployment as the referencing process.
* `latest`, which will refer to the latest deployed version of the CadenzaFlow Form.
*  `version`, which allows you to specify a specific version to be referenced from the BPMN element with the `cadenzaflow:formRefVersion` attribute.

```xml
<bpmn:userTask
    id="myUserTask"
    cadenzaflow:formRef="formId"
    cadenzaflow:formRefBinding="version"
    cadenzaflow:formRefVersion="1">
</bpmn:userTask>
```

The attributes `cadenzaflow:formRef` and `cadenzaflow:formRefVersion` can be specified as an expression which will be evaluated on execution of the task or start event.
...
<bpmn:userTask
    ...
    cadenzaflow:formRef="${formId}"
    cadenzaflow:formRefBinding="version"
    cadenzaflow:formRefVersion="${formVersion}">
</bpmn:userTask>
```

{{< img src="img/reference-cadenzaflow-form.png" title="Provide Form Key for CadenzaFlow Forms" >}}

## Form Key

Aa an alternative to `formRef` you can reference a CadenzaFlow Form file with a `deployment` or `app` [form key]({{< ref "/user-guide/task-forms/_index.md#form-key-details" >}}):

* `cadenzaflow-forms:deployment:FORM_NAME.form`
* `cadenzaflow-forms:app:forms/FORM_NAME.form`
...
{{< img src="img/variable-mapping-cadenzaflow-form.png" title="User Input/Output Mappings for default values for form fields" >}}
...
Like this, CadenzaFlow Forms show available options dynamically based on process data (variables).

To bind a variable to a dynamic component, define its name in CadenzaFlow Modeler's form builder in the Properties Panel under **Options Source** -> **Type** -> **Input Data** -> **Dynamic options** -> **Input values key** for the respective component.

CadenzaFlow Forms support the following variable types that can represent JSON:

* `Json`
* `Object` with the `serializationDataFormat: application/json`

CadenzaFlow Forms store and retrieve user selections for each component in a variable whose name equals the component key.
...
## Deployment
If you want to include your CadenzaFlow Form as part of the `deployment`, then you need to deploy the `.form` file in the same deployment as the respective `.bpmn` diagram - for example using the CadenzaFlow Modeler (since Modeler Version 5.0.0).

{{< note title="Automatic deployment" class="warning" >}}
CadenzaFlow Forms are not automatically deployed as part of a [process archive]({{< ref "/reference/deployment-descriptors/tags/process-archive.md" >}}) by default. 
...
Using [CadenzaFlow Run]({{< ref "/user-guide/cadenzaflow-bpm-run.md#starting-with-cadenzaflow-platform-run" >}}), all additional resources - including CadenzaFlow Forms - 
placed inside the `configuration/resources/` directory are automatically deployed.
{{< /note >}}

{{< img src="img/deploy-form.png" title="Deploy your CadenzaFlow Form file" >}}

You can also include CadenzaFlow Forms from other deployments by using [form references](#form-reference).

# External Task Forms
{{< note title="Using Task Forms outside of CadenzaFlow Tasklist" class="info" >}}
...
```xml
<userTask id="theTask" cadenzaflow:formKey="app:FORM_NAME.html"
          cadenzaflow:candidateUsers="John, Mary"
          name="my Task">
```
...
{{< note title="CadenzaFlow Forms or Generated Task Forms?" class="info" >}}
  The feature set of CadenzaFlow Forms and Generated Task Forms are similar. For new projects, we recommend to use CadenzaFlow Forms, as they offer more flexibility and are easier to create.
{{< /note >}}


The CadenzaFlow process engine supports generating HTML task forms based on Form Data Metadata provided in the BPMN 2.0 XML. Form Data Metadata is a set of BPMN 2.0 vendor extensions provided by CadenzaFlow, allowing you to define form fields directly in the BPMN 2.0 XML:

```xml
<userTask id="usertask" name="Task">
  <extensionElements>
    <cadenzaflow:formData>
        <cadenzaflow:formField
            id="firstname" label="First Name" type="string">
            <cadenzaflow:validation>
               <cadenzaflow:constraint name="maxlength" config="25" />
               <cadenzaflow:constraint name="required" />
            </cadenzaflow:validation>
        </cadenzaflow:formField>
        <cadenzaflow:formField
...
Form metadata can be graphically edited using the [CadenzaFlow Modeler](https://cadenzaflow.com/products/cadenzaflow-platform/modeler/).

This form would look like this in Tasklist:

{{< img src="img/generated-forms-example.png" title="Generated Form" >}}

As you can see, the `<cadenzaflow:formData ... />` element is provided as a child element of the BPMN `<extensionElements>` element. Form metadata consists of multiple form fields which represent individual input fields where a user has to provide some value or selection.
...
Validation can be used for specifying frontend and backend validation of form fields. CadenzaFlow provides a set of built-in form field validators and an extension point for plugging in custom validators.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <p>
          <code>
            &lt;cadenzaflow:constraint name=&quot;minlength&quot; config=&quot;4&quot; /&gt;
          </code>
        </p>
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        ...
        <p>
          <code>
            &lt;cadenzaflow:constraint name=&quot;min&quot; config=&quot;1000&quot; /&gt;
          </code>
        </p>
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        <p>
          <code>
            &lt;cadenzaflow:constraint name=&quot;max&quot; config=&quot;10000&quot; /&gt;
          </code>
        </p>
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        <p>
          <code>
            &lt;cadenzaflow:constraint name=&quot;readonly&quot; /&gt;
          </code>
        </p>
      </td>
    </tr>
...
CadenzaFlow supports custom validators. Custom validators are referenced using their fully qualified classname or an expression. Expressions can be used for resolving Spring or CDI @Named beans:

```xml
<cadenzaflow:formField
    id="firstname" label="First Name" type="string">
    <cadenzaflow:validation>
       <cadenzaflow:constraint name="validator" config="com.asdf.MyCustomValidator" />
       <cadenzaflow:constraint name="validator" config="${validatorBean}" />
    </cadenzaflow:validation>
</cadenzaflow:formField>
```
...
A custom validator implements the `org.cadenzaflow.bpm.engine.impl.form.validator.FormFieldValidator` interface:
