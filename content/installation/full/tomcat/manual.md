...
This section describes how you can install CadenzaFlow and its components on a vanilla [Apache Tomcat](http://tomcat.apache.org/), if you are not able to use the pre-packaged Tomcat distribution. In addition, download a [Tomcat distribution](https://downloads.cadenzaflow.com/release/communityEdition/cadenzaflow-bpm/tomcat/) or [Enterprise Edition Tomcat distribution](https://downloads.cadenzaflow.com/release/enterpriseEdition/cadenzaflow-bpm/tomcat/) to fetch the required CadenzaFlow modules.
...
* `$PLATFORM_VERSION` denotes the version of CadenzaFlow you want to install or already have installed, e.g. `1.0.0`.
* `$TOMCAT_DISTRIBUTION` represents the downloaded pre-packaged CadenzaFlow distribution for Tomcat, e.g. `cadenzaflow-bpm-tomcat-$PLATFORM_VERSION.zip` or `cadenzaflow-bpm-tomcat-$PLATFORM_VERSION.tar.gz`.
...
* [CallActivityContextSwitchTest](https://github.com/cadenzaflow/cadenzaflow-bpm-platform/blob/f37877b822dabcbf3cee5806bd5833d18cdcb543/qa/integration-tests-engine/src/test/java/org/cadenzaflow/bpm/integrationtest/functional/context/CallActivityContextSwitchTest.java)
* [CdiBeanCallActivityResolutionTest](https://github.com/cadenzaflow/cadenzaflow-bpm-platform/blob/f37877b822dabcbf3cee5806bd5833d18cdcb543/qa/integration-tests-engine/src/test/java/org/cadenzaflow/bpm/integrationtest/functional/cdi/CdiBeanCallActivityResolutionTest.java)
{{< /note >}}


# Setup

Before you can install the CadenzaFlow components, you need to perform a number of required setup steps.
...
* Create a database schema for CadenzaFlow yourself.
...
Add the entry `org.cadenzaflow.bpm.container.impl.tomcat.TomcatBpmPlatformBootstrap` as Listener before the `GlobalResourcesLifecycleListener` in your `$TOMCAT_HOME/conf/server.xml`. This class is responsible for starting and stopping CadenzaFlow as Tomcat is started and stopped.

```xml
<Server port="8005" shutdown="SHUTDOWN">
  ...
  <Listener className="org.cadenzaflow.bpm.container.impl.tomcat.TomcatBpmPlatformBootstrap" />
  ...
```
...
<Server>
  ...
  <GlobalNamingResources>
    ...
    <Resource name="jdbc/ProcessEngine"
              ...
              url="jdbc:h2:./cadenzaflow-h2-dbs/process-engine;TRACE_LEVEL_FILE=0"
...
```xml
     <Resource name="global/cadenzaflow-bpm-platform/process-engine/ProcessEngineService!org.cadenzaflow.bpm.ProcessEngineService" auth="Container"
              type="org.cadenzaflow.bpm.ProcessEngineService"
              description="CadenzaFlow Platform Process Engine Service"
              factory="org.cadenzaflow.bpm.container.impl.jndi.ProcessEngineServiceObjectFactory" />

    <Resource name="global/cadenzaflow-bpm-platform/process-engine/ProcessApplicationService!org.cadenzaflow.bpm.ProcessApplicationService" auth="Container"
              type="org.cadenzaflow.bpm.ProcessApplicationService"
              description="CadenzaFlow Platform Process Application Service"
              factory="org.cadenzaflow.bpm.container.impl.jndi.ProcessApplicationServiceObjectFactory" />
```
...
This section describes how to install optional CadenzaFlow dependencies onto a Tomcat server. None of these are required to work with the core platform.
...
1. Download the CadenzaFlow web application that contains both applications from our [Artifact Repository](https://downloads.cadenzaflow.com/artifacts/cadenzaflow-bpm/org/cadenzaflow/bpm/webapp/cadenzaflow-webapp-tomcat/).
   Or switch to the private repository for the enterprise version (User and password from license required).
    * For [Tomcat 10](https://downloads.cadenzaflow.com/artifacts/cadenzaflow-bpm/org/cadenzaflow/bpm/webapp/cadenzaflow-webapp-tomcat-jakarta/), the name of the artifact is `$PLATFORM_VERSION/cadenzaflow-webapp-tomcat-jakarta-$PLATFORM_VERSION.war`.
    * For [Tomcat 9](https://downloads.cadenzaflow.com/artifacts/cadenzaflow-bpm/org/cadenzaflow/bpm/webapp/cadenzaflow-webapp-tomcat/), the name of the artifact is `$PLATFORM_VERSION/cadenzaflow-webapp-tomcat-$PLATFORM_VERSION.war`.
2. Copy the war file to `$TOMCAT_HOME/webapps/cadenzaflow.war`.
   Optionally you may name it differently or extract it to a folder to deploy it to a different context path.
3. Startup Tomcat.
4. Access Cockpit and Tasklist via `/cadenzaflow/app/cockpit` and `/cadenzaflow/app/tasklist` or under the context path you configured.
...
1. Download the REST API web application archive from our [Artifact Repository](https://downloads.cadenzaflow.com/artifacts/cadenzaflow-bpm/org/cadenzaflow/bpm/cadenzaflow-engine-rest/).
    Or switch to the private repository for the enterprise version (User and password from license required).
    * For [Tomcat 10](https://downloads.cadenzaflow.com/artifacts/cadenzaflow-bpm/org/cadenzaflow/bpm/cadenzaflow-engine-rest-jakarta/), the name of the artifact is `$PLATFORM_VERSION/cadenzaflow-engine-rest-jakarta-$PLATFORM_VERSION-tomcat.war`.
    * For [Tomcat 9](https://downloads.cadenzaflow.com/artifacts/cadenzaflow-bpm/org/cadenzaflow/bpm/cadenzaflow-engine-rest/), the name of the artifact is `$PLATFORM_VERSION/cadenzaflow-engine-rest-$PLATFORM_VERSION-tomcat.war`.
...
In order to activate CadenzaFlow Connect functionality for a process engine, a process engine plugin has to be registered in `$TOMCAT_HOME/conf/bpm-platform.xml` as follows:
...
In order to activate CadenzaFlow Spin functionality for a process engine, a process engine plugin has to be registered in `$TOMCAT_HOME/conf/bpm-platform.xml` as follows:
...
<bpm-platform ...>
  ...
  <process-engine name="default">
    ...
    <plugins>
      ... existing plugins ...
      <plugin>
        <class>org.cadenzaflow.spin.plugin.impl.SpinProcessEnginePlugin</class>
      </plugin>
    </plugins>
    ...
  </process-engine>
