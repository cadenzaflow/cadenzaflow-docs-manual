...
Make sure the `cadenzaflow-identity-ldap-$PLATFORM_VERSION.jar` is present in the
...
Users and Groups in the CadenzaFlow webapplication. In order to activate this, add the following
configuration to the `web.xml` file of CadenzaFlow webapplication
(`cadenzaflow-webapp-tomcat-$PLATFORM_VERSION.war/WEB-INF/web.xml`):
...
<filter>
  ...
  <filter-class>org.cadenzaflow.bpm.webapp.impl.security.filter.SessionCookieFilter</filter-class>
</filter>
<filter-mapping>
  <filter-name>SessionCookieFilter</filter-name>
...
<filter>
  ...
  <filter-class>
    org.cadenzaflow.bpm.webapp.impl.security.filter.headersec.HttpHeaderSecurityFilter
  </filter-class>
</filter>
