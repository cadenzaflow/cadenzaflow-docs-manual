...
menu:
  main:
    ...
    pre: "Install and update the CadenzaFlow database schema used by the Process Engine."

---

This document will guide you through the installation and update of the CadenzaFlow [database schema]({{< ref "/user-guide/process-engine/database/database-schema.md" >}}) used by the process engine.
...
Consult the installation guide related to your setup to configure the database for a [remote engine]({{< ref "/installation/cadenzaflow-bpm-run.md" >}}), [shared engine]({{< ref "/installation/full/_index.md" >}}), or [embedded engine]({{< ref "/installation/spring-boot.md" >}}) accordingly.
...
CadenzaFlow supports a variety of databases that are documented in the [supported environments]({{< ref "/introduction/supported-environments.md#databases" >}}). 

CadenzaFlow supports the following ways of installing the database schema:

* Use the database migration tool [Liquibase](https://www.liquibase.org/) with the provided changelog for a semi-automatic installation and update. Liquibase keeps track of database schema changes. This allows you to focus on *when* changes should be applied rather than also on *which* changes are relevant. Starting with version `1.0.0`, CadenzaFlow comes with a curated *changelog file* that Liquibase can consume.
...
READ COMMITED is the required isolation level for database systems to run CadenzaFlow with. 
You may have to change the default setting on your database when installing CadenzaFlow. 
...
You can either install your database schema using Liquibase or using the provided SQL scripts manually. You can switch between those mechanisms when updating your CadenzaFlow version at a later stage if desired.
...
CadenzaFlow comes with a maintained changelog file that Liquibase can consume.
This changelog defines which SQL statements to execute on a database.
You can find the changelog and its referenced resources on our [Artifact Repository](https://nexus.cadenzaflow.com/service/rest/repository/browse/cadenzaflow-nexus/org/cadenzaflow/bpm/distro/cadenzaflow-sql-scripts/). .
Select the respective version (`$PLATFORM_VERSION`) and download the resources as a `zip` or `tar.gz` file.
Open the `cadenzaflow-sql-scripts-$PLATFORM_VERSION/liquibase` folder to find the changelog. 
...
2. Run Liquibase's [update command](https://docs.liquibase.com/commands/community/update.html) referencing the `cadenzaflow-changelog.xml`.
...
To install the database schema required for CadenzaFlow, we provide a set of scripts with prepared DDL statements.
Those scripts create all required tables and default indices. You can find the provided SQL scripts on our [Artifact Repository](https://nexus.cadenzaflow.com/service/rest/repository/browse/cadenzaflow-nexus/org/cadenzaflow/bpm/distro/cadenzaflow-sql-scripts/). 
Select the respective version (`$PLATFORM_VERSION`) and download the scripts as a `zip` or `tar.gz` file.
Open the `cadenzaflow-sql-scripts-$PLATFORM_VERSION/create` folder to find all available scripts. 
In case you are using a [pre-packaged distribution]({{< ref "/introduction/downloading-cadenzaflow.md/#full-distribution" >}}), the SQL scripts already reside in the `sql/create` folder of the distribution.
...
Updating to a newer CadenzaFlow minor version also requires a database schema update. You can reuse all the options available for a [schema installation](#installation) here as well.
...
In case you are just updating to a newer patch level of your CadenzaFlow installation, a schema update might not be necessary.
...
Open the `cadenzaflow-sql-scripts-$Y/liquibase` folder to find the changelog file.
...
1. Run Liquibase's [update command](https://docs.liquibase.com/commands/community/update.html) referencing the new `cadenzaflow-changelog.xml` of version `$Y`.
...
1. Run Liquibase's [changelogSyncToTag command](https://docs.liquibase.com/commands/community/changelogsynctotag.html) referencing the `cadenzaflow-changelog.xml` and using your current database schema version as the tag.
You can pass on the connection details to your database instance via parameters as described in the Liquibase documentation or [create a properties file](https://docs.liquibase.com/workflows/liquibase-community/creating-config-properties.html).
...
1. Check for [available database patch scripts](https://nexus.cadenzaflow.com/service/rest/repository/browse/cadenzaflow-nexus/org/cadenzaflow/bpm/distro/cadenzaflow-sql-scripts/) for your database that are within the bounds of your update path.
...
Open the `cadenzaflow-sql-scripts-$Y/upgrade` folder to find all available scripts. 
...
This section explains how to perform a [patch-level update]({{< ref "/update/cadenzaflow-patch/patch-level.md" >}}) for your database schema. The *patch level* is the version number "after the second dot". For example, update from `1.0.1` to `1.0.2`.
...
Between patch levels, the structure of the database schema does not change. The database structure of all patch releases is backward compatible with the corresponding minor version. For example, the database schema of all `1.x.x` versions are backward compatible with the `1.0.0` schema.

The one exception to this is a bug in the database schema itself. If you are affected by such a bug, you have the option to run a patch script. The following list contains all available patch scripts, information on what the fixes are related to, and a link to the corresponding [CadenzaFlow Jira](https://jira.cadenzaflow.com/projects/CAM) issue:
