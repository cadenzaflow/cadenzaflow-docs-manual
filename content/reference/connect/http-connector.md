...
In CadenzaFlow Connect a `Connectors` class exists which automatically detects
...
CadenzaFlow Connect HTTP client uses the Apache HTTP client to make HTTP requests. Accordingly, it supports the same configuration options.
...
```java
package org.cadenzaflow.connect.example;
...
`org.cadenzaflow.connect.spi.ConnectorConfigurator` to your
...
request.setRequestParameter("url", "http://cadenzaflow.org");
