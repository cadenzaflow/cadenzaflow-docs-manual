...
`org.cadenzaflow.connect.spi.ConnectorConfigurator` can be implemented.  A
...
* Provide a custom implementation of `org.cadenzaflow.connect.spi.ConnectorConfigurator`
* Add the configurator's fully qualified classname to a file named `META-INF/services/org.cadenzaflow.connect.spi.ConnectorConfigurator`
...
`org.cadenzaflow.connect.spi.Connector`. An implementation of this interface can be
registered by implementing the SPI `org.cadenzaflow.connect.spi.ConnectorProvider`.
...
* Provide a custom implementation of `org.cadenzaflow.connect.spi.Connector`
* Provide a custom implementation of `org.cadenzaflow.connect.spi.ConnectorProvider`
* Add the provider's fully qualified classname to a file named `META-INF/services/org.cadenzaflow.connect.spi.ConnectorProvider`
* Ensure that the artifact containing the provider is reachable from Connect's classloader

If you now call `org.cadenzaflow.connect.Connectors.getAvailableConnectors()`, then
the custom connector is returned along with the built-in connectors.
Furthermore, `org.cadenzaflow.connect.Connectors.getConnector(String connectorId)`
