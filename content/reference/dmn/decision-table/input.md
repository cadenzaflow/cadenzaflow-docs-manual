...
CadenzaFlow to reference the input in the history of evaluated
decisions. Therefore, it is required by the CadenzaFlow DMN engine. It is set as
...
<definitions id="definitions"
             ...
             namespace="http://cadenzaflow.org/schema/1.0/dmn">
  <!-- ... -->
</definitions>
```
...
The name of the variable can be specified by the `cadenzaflow:inputVariable`
[extension attribute][inputVariable] on the `input` element. By default, the
name is `cellInput`.

To use the attribute you have to define the CadenzaFlow DMN namespace
`xmlns:cadenzaflow="http://cadenzaflow.org/schema/1.0/dmn` in the XML.

```xml
<definitions id="definitions"
             name="definitions"
             xmlns="https://www.omg.org/spec/DMN/20191111/MODEL/"
             xmlns:cadenzaflow="http://cadenzaflow.org/schema/1.0/dmn"
             namespace="http://cadenzaflow.org/schema/1.0/dmn">
  <decision id="dish" name="Dish">
    <decisionTable id="decisionTable">
      <input id="input1"
             label="Season"
             cadenzaflow:inputVariable="currentSeason">
        <!-- ... -->
      </input>
      <!-- ... -->
    </decisionTable>
...
[inputVariable]: {{< ref "/reference/dmn/custom-extensions/cadenzaflow-attributes.md#inputvariable" >}}
