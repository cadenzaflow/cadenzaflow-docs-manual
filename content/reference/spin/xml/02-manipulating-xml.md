...
String xml = "<order xmlns:cam=\"http://cadenzaflow.org/example\" id=\"order1\" cam:name=\"name\" />";

boolean hasAttr = XML(xml).hasAttr("id");
assertTrue(hasAttr);

hasAttr = XML(xml).hasAttrNs("http://cadenzaflow.org/example", "id");
assertTrue(hasAttr);
```
...
String xml = "<order xmlns:cam=\"http://cadenzaflow.org/example\" id=\"order1\" cam:name=\"order1\" />";

SpinXmlDomAttribute attribute = XML(xml).attrNs("http://cadenzaflow.org/example", "name");
```
...
attributes = XML(xml).attrs("http://cadenzaflow.org/example");
```
...
names = XML(xml).attrNames("http://cadenzaflow.org/example");
```
...
XML(xml).attrNs("http://cadenzaflow.org/example", "name", "newName");
...
SpinXmlDomElement element = XML(xml).removeAttrNs("http://cadenzaflow.org/example", "name");
assertFalse(element.hasAttrNs("http://cadenzaflow.org/example/", "name"));
...
String xml = "<order xmlns:cam=\"http://cadenzaflow.org/example\">" +
      "<date/><cam:due/><item/><item/><cam:op/><cam:op/></order>";

SpinXmlDomElement date = XML(xml).childElement("date");
SpinXmlDomElement due = XML(xml).childElement("http://cadenzaflow.org/example", "due");

SpinCollection<SpinXmlDomElement> items = XML(xml).childElements("item");
SpinCollection<SpinXmlDomElement> ops = XML(xml).childElements("http://cadenzaflow.org/example", "ops");
```
