...
For example, a custom application may provide an implementation of `org.cadenzaflow.spin.spi.DataFormatConfigurator` that exchanges 
the `JAXBContext`. Spin uses and caches the context to improve performance.

The data format class to register a configurator for is `org.cadenzaflow.spin.impl.xml.dom.format.DomXmlDataFormat`. 
