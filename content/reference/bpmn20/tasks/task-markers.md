...
```xml
<userTask id="miTasks" name="My Task ${loopCounter}" cadenzaflow:assignee="${assignee}">
...
```xml
<userTask id="miTasks" name="My Task" cadenzaflow:assignee="${assignee}">
  <multiInstanceLoopCharacteristics isSequential="true"
     cadenzaflow:collection="${myService.resolveUsersForTask()}" cadenzaflow:elementVariable="assignee" >
  </multiInstanceLoopCharacteristics>
</userTask>
```
...
<userTask id="miTasks" name="My Task" cadenzaflow:assignee="${assignee}">
  <multiInstanceLoopCharacteristics isSequential="false"
     cadenzaflow:collection="assigneeList" cadenzaflow:elementVariable="assignee" >
...
<table class="table table-striped">
  <tr>
    <th>Attributes</th>
    <td>
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#collection" >}}">cadenzaflow:collection</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#elementvariable" >}}">cadenzaflow:elementVariable</a>,
...
JSON Arrays created with [CadenzaFlow SPIN]({{< ref "/reference/spin/_index.md">}}) can be used as a collection for multi-instance activities. 
...
We can now use `collection` variable in multi-instance activity's `cadenzaflow:collection` extension element.

```xml
<multiInstanceLoopCharacteristics 
  cadenzaflow:collection="${collection.prop('collection').elements()}" 
  cadenzaflow:elementVariable="collectionElem" />
```
...
```xml
<serviceTask id="undoBookHotel" isForCompensation="true" cadenzaflow:class="..." />
```


# Additional Resources

* [Tasks](http://cadenzaflow.org/bpmn/reference.html#activities-task) in the [BPMN 2.0 Modeling Reference](http://cadenzaflow.org/bpmn/reference.html)
