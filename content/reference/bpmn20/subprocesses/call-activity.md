...
<callActivity id="callSubProcess" calledElement="checkCreditProcess"
  cadenzaflow:calledElementBinding="latest|deployment|version"
  cadenzaflow:calledElementVersion="17">
</callActivity>
```
or
```xml
<callActivity id="callSubProcess" calledElement="checkCreditProcess"
  cadenzaflow:calledElementBinding="versionTag"
  cadenzaflow:calledElementVersionTag="ver-tag-1.0.1">
</callActivity>
```
...
The `cadenzaflow:calledElementTenantId` attribute allows to explicitly specify a tenant id:

```xml
<callActivity id="callSubProcess" calledElement="checkCreditProcess"
  cadenzaflow:calledElementTenantId="TENANT_1">
</callActivity>
```
...
```xml
<callActivity id="callSubProcess" calledElement="checkCreditProcess"
  cadenzaflow:calledElementTenantId="${ myBean.calculateTenantId(variable) }">
</callActivity>
```
...
```xml
<callActivity id="callSubProcess" calledElement="checkCreditProcess"
  cadenzaflow:calledElementTenantId="${ execution.tenantId }">
</callActivity>
```
...
<callActivity id="callSubProcess" calledElement="checkCreditProcess" >
  <extensionElements>
    <cadenzaflow:in source="someVariableInMainProcess" target="nameOfVariableInSubProcess" />
    <cadenzaflow:out source="someVariableInSubProcss" target="nameOfVariableInMainProcess" />
  </extensionElements>
</callActivity>
```
...
<callActivity id="callSubProcess" calledElement="checkCreditProcess" >
  <extensionElements>
    <cadenzaflow:in variables="all" />
    <cadenzaflow:out variables="all" />
  </extensionElements>
</callActivity>
```
...
<callActivity id="callSubProcess" calledElement="checkCreditProcess" >
  <extensionElements>
    <cadenzaflow:in sourceExpression="${x+5}" target="y" />
    <cadenzaflow:out sourceExpression="${y+5}" target="z" />
  </extensionElements>
</callActivity>
```
...
<callActivity id="callSubProcess" calledElement="checkCreditProcess" >
  <extensionElements>
    ...
    <cadenzaflow:inputOutput>
      <cadenzaflow:inputParameter name="var1">
      ...
      <cadenzaflow:inputParameter name="var2"></cadenzaflow:inputParameter>
    </cadenzaflow:inputOutput>

    <!-- Mapping to called instance -->
    <cadenzaflow:in variables="all" local="true" />
  </extensionElements>
</callActivity>
```
...
<callActivity id="callSubProcess" calledElement="checkCreditProcess" >
  <extensionElements>
    <!-- Input/Output parameters -->
    <cadenzaflow:inputOutput>
      <cadenzaflow:outputParameter name="var1">
        <cadenzaflow:script scriptFormat="groovy">
          <![CDATA[
            sum = a + b + c
          ]]>
      ...
      <cadenzaflow:outputParameter name="var2"></cadenzaflow:outputParameter>
    </cadenzaflow:inputOutput>

    <!-- Mapping from called instance -->
    <cadenzaflow:out variables="all" local="true" />
  </extensionElements>
</callActivity>
```

When the called process instance ends, due to `local="true"` in the `cadenzaflow:out` parameter all variables are mapped to local variables of the execution executing the call activity. These variables can be mapped to process instance variables by using an output mapping. Any variable that is not declared by a `cadenzaflow:outputParameter` element will not be available anymore after the call activity ends.
...
The first one is to set the CadenzaFlow extension property `variableMappingClass` and reference the implementation of the `DelegateVariableMapping` interface via the whole class name.
...
```xml
 <process id="callSimpleSubProcess">
    ...
    <callActivity id="callSubProcess" calledElement="simpleSubProcess" cadenzaflow:variableMappingClass="org.cadenzaflow.bpm.example.bpm.callactivity.DelegatedVarMapping"/>
...
The second one is to set the CadenzaFlow extension property `variableMappingDelegateExpression` with an expression.
...
```xml
  <process id="callSimpleSubProcess">
    ...
    <callActivity id="callSubProcess" calledElement="simpleSubProcess" cadenzaflow:variableMappingDelegateExpression="${expr}"/>
...
<callActivity id="callSubProcess" calledElement="checkCreditProcess" >
  <extensionElements>
    <cadenzaflow:in businessKey="#{execution.processBusinessKey}" />
  </extensionElements>
</callActivity>
```
...
```xml
<callActivity id="callSubProcess" cadenzaflow:caseRef="checkCreditCase"
  cadenzaflow:caseBinding="latest|deployment|version"
  cadenzaflow:caseVersion="17">
</callActivity>
```
...
In order to override the default behavior, the tenant id for resolving the called case definition can be specified explicitly using the `cadenzaflow:caseTenantId` attribute:

```xml
<callActivity id="callSubProcess" cadenzaflow:caseRef="checkCreditCase"
  cadenzaflow:caseTenantId="TENANT_1">
</callActivity>
```
...
```xml
<callActivity id="callSubProcess" cadenzaflow:caseRef="checkCreditCase"
  cadenzaflow:caseTenantId="${ myBean.calculateTenantId(variable) }">
</callActivity>
```
...
```xml
<callActivity id="callSubProcess" cadenzaflow:caseRef="checkCreditCase"
  cadenzaflow:caseTenantId="${ execution.tenantId }">
</callActivity>
```
...
<table class="table table-striped">
  <tr>
    ...
    <td>
      ...
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#calledelementbinding" >}}">cadenzaflow:calledElementBinding</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#calledelementversion" >}}">cadenzaflow:calledElementVersion</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#calledelementversiontag" >}}">cadenzaflow:calledElementVersionTag</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#calledelementtenantid" >}}">cadenzaflow:calledElementTenantId</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#casebinding" >}}">cadenzaflow:caseBinding</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#caseref" >}}">cadenzaflow:caseRef</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#caseversion" >}}">cadenzaflow:caseVersion</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#casetenantid" >}}">cadenzaflow:caseTenantId</a>,
      ...
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#variablemappingclass" >}}">cadenzaflow:variableMappingClass</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-attributes.md#variablemappingdelegateexpression" >}}">cadenzaflow:variableMappingDelegateExpression</a>
    </td>
  </tr>
  <tr>
    <th>Extension Elements</th>
    <td>
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-elements.md#in" >}}">cadenzaflow:in</a>,
      <a href="{{< ref "/reference/bpmn20/custom-extensions/extension-elements.md#out" >}}">cadenzaflow:out</a>,
  ...
  <tr>
    <td></td>
    <td>
      The attribute <code>cadenzaflow:calledElementVersion</code> should only be set if
      the attribute <code>cadenzaflow:calledElementBinding</code> equals <code>version</code>
    </td>
  </tr>
  <tr>
    <td></td>
    <td>
      ...
      with the attribute <code>cadenzaflow:caseRef</code> and vice versa.
    </td>
  </tr>
  <tr>
    <td></td>
    <td>
      The attribute <code>cadenzaflow:caseVersion</code> should only be set if
      the attribute <code>cadenzaflow:caseBinding</code> equals <code>version</code>
    </td>
  </tr>
</table>
...
*   [Call Activity](http://cadenzaflow.org/bpmn/reference.html#activities-call-activity) in the [BPMN 2.0 Modeling Reference](http://cadenzaflow.org/bpmn/reference.html)
