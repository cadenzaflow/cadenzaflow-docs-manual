...
The transition in which the repetition rule is evaluated can be changed by a CadenzaFlow extension attribute named `cadenzaflow:repeatOnStandardEvent`. For a task it looks as follows:
...
<definitions>
  <case id="case" name="Case">
    <casePlanModel id="CasePlanModel_1">
      ...
      <humanTask id="HumanTask">
        <defaultControl>
          <repetitionRule cadenzaflow:repeatOnStandardEvent="disable">
            <condition><![CDATA[${score < 50}]]></condition>
          </repetitionRule>
        </defaultControl>
      </humanTask>
...
<table class="table table-striped">
  <tr>
    <th>Attributes</th>
    <td>
      <a href="{{< ref "/reference/cmmn11/custom-extensions/cadenzaflow-attributes.md#repeatonstandardevent" >}}">cadenzaflow:repeatOnStandardEvent</a>
    </td>
  </tr>
  <tr>
    <th>Note</th>
    <td>
      The attribute <code>cadenzaflow:repeatOnStandardEvent</code> is ignored when a milestone, stage or task has at least one entry criterion.
    </td>
  </tr>
</table>
