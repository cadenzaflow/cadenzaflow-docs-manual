...
By default, the process task creates a new process instance of the latest process definition with the specified key. To specify a different version of a process, it is possible to define a binding with the CadenzaFlow custom attribute `processBinding`. The following values are allowed for the attribute `processBinding`:
...
<processTask id="checkCreditProcess" processRef="checkCreditProcess"
  cadenzaflow:processBinding="version"
  cadenzaflow:processVersion="3">
</processTask>
```
...
The `cadenzaflow:processTenantId` attribute allows to explicitly specify a tenant id:

```xml
<processTask id="checkCreditProcess" processRef="checkCreditProcess"
  cadenzaflow:processTenantId="TENANT_1">
</processTask>
```
...
```xml
<processTask id="checkCreditProcess" processRef="checkCreditProcess"
  cadenzaflow:processTenantId="${ myBean.calculateTenantId(variable) }">
</processTask>
```
...
```xml
<processTask id="checkCreditProcess" processRef="checkCreditProcess"
  cadenzaflow:processTenantId="${ caseExecution.tenantId }">
</processTask>
```

# Exchange Variables

The CadenzaFlow custom extensions elements `in` and `out` allow to exchange variables between the process task (in a case instance) and the process instance that it creates: `in` elements of a process task map case variables to input variables of the launched process instance and `out` mappings of a process task map output variables of the process instance to case variables, e.g.,:

```xml
<processTask id="checkCreditProcess" name="Check credit" processRef="checkCreditProcess">
  <extensionElements>
    <cadenzaflow:in source="aCaseVariable" target="aProcessVariable" />
    <cadenzaflow:out source="aProcessVariable" target="anotherCaseVariable" />
  </extensionElements>
</processTask>
```
...
It is possible to decide at runtime which variables are mapped into the called process instance. This can be declared with the `local` attribute on the `cadenzaflow:in` element as follows:
...
<processTask id="checkCreditProcess" name="Check credit" processRef="checkCreditProcess">
  <extensionElements>
    <cadenzaflow:in businessKey="#{caseExecution.caseBusinessKey}" />
  </extensionElements>
</processTask>
```
...
<processTask id="checkCreditProcess" name="Check credit" processRef="checkCreditProcess">
  <extensionElements>
    <cadenzaflow:in businessKey="#{customerId}" />
  </extensionElements>
</processTask>
```
...
<table class="table table-striped">
  <tr>
    <th>Attributes</th>
    <td>
      <a href="{{< ref "/reference/cmmn11/custom-extensions/cadenzaflow-attributes.md#processbinding" >}}">cadenzaflow:processBinding</a>,
      <a href="{{< ref "/reference/cmmn11/custom-extensions/cadenzaflow-attributes.md#processtenantid" >}}">cadenzaflow:processTenantId</a>,
      <a href="{{< ref "/reference/cmmn11/custom-extensions/cadenzaflow-attributes.md#processversion" >}}">cadenzaflow:processVersion</a>
    </td>
  </tr>
  <tr>
    <th>Extension Elements</th>
    <td>
      <a href="{{< ref "/reference/cmmn11/custom-extensions/cadenzaflow-elements.md#in" >}}">cadenzaflow:in</a>,
      <a href="{{< ref "/reference/cmmn11/custom-extensions/cadenzaflow-elements.md#out" >}}">cadenzaflow:out</a>,
  ...
  <tr>
    <th>Constraints</th>
    <td>
      The attribute <code>cadenzaflow:processVersion</code> should only be set if
      the attribute <code>cadenzaflow:processBinding</code> is equal to <code>version</code>
    </td>
  </tr>
</table>
