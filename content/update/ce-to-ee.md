...
If you’re hoping to utilize [features of Enterprise Edition](https://cadenzaflow.com/platform-7/editions/) (such as see how many process instances have been run, token history, heatmaps, and migrate model versions) you may be interested in migrating from CadenzaFlow Community Edition to the Enterprise Edition.

In this guide, we’ll step through migration from CadenzaFlow Community Edition to Enterprise Edition.

# Prerequisites

- IDE or code editor of your choice (in this example, we’ll use Visual Studio Code.)
- Access to [CadenzaFlow Modeler](https://cadenzaflow.com/download/).
- Access to [GitHub](https://github.com/cadenzaflow-community-hub/CadenzaFlow-7-Spring-Boot-Tutorial-Lafayette) for our tutorial example.
- Java 17
- Maven

{{< note class="info" >}}
If needed, you can obtain a license key and [start a 30-day trial](https://cadenzaflow.com/download/enterprise/) of CadenzaFlow Enterprise Edition.
{{< /note >}}
...
1. Clone our [example repository](https://github.com/cadenzaflow-community-hub/CadenzaFlow-7-Spring-Boot-Tutorial-Lafayette), where we’ll take part in assisting some mock revolutions.
...
{{< img src="../img/cadenzaflow-cockpit-example.png" title="CadenzaFlow Cockpit" >}}

## Migration

To officially begin migrating from CadenzaFlow Community Edition to the Enterprise Edition, take the following steps:
...
4. Add `-ee` to the version. For example, `<version>7.16.0-ee</version>`. You’ll need to make these changes below the following `artifactIds`: `cadenzaflow-bom`, `cadenzaflow-bpm-spring-boot-starter-rest`, `cadenzaflow-bpm-spring-boot-starter-webapp` (you’ll also adjust this particular artifactID itself to `cadenzaflow-bpm-spring-boot-starter-webapp-ee`.)
    ```xml
    <dependency>
...
5. We need to tell it where to find these new dependencies. Therefore, we’ll add the locations of the repositories at the end of the dependencies in the `pom.xml` file as follows:
    ...
    <repositories>
      ...
      <repository>
        <id>Cadenzaflow-bpm-nexus-ee</id>
        <name>cadenzaflow-bpm-nexus</name>
        <url>
        https://artifacts.cadenzaflow.com/artifactory/private/
        </url>
      </repository>
    </repositories>
    ```
...
Now that you’ve made the physical adjustments in your IDE to update to Enterprise Edition, let’s make a few final changes in CadenzaFlow.
...
5. Paste your license key into the prompted text field, or if you haven’t already, obtain a license key and [start a 30-day trial](https://cadenzaflow.com/download/enterprise/) of CadenzaFlow Enterprise Edition.
6. Click **Save Key**.

# Next steps

After you’ve made the required changes above, return to CadenzaFlow Cockpit. You’ll notice the migration has completed as you can now view the section titled **Metrics**.
